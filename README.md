...
2. Configure a path-scoped Cloudflare Access application for `/api` and `/api/*` (or the exact equivalent paths in the Access policy). Keep the public shell and client routes public so `/`, `/groups/*`, and other SPA navigation can render the signed-out landing shell. The sign-in bootstrap is `GET /api/auth/bootstrap`, intentionally under the protected path; the top-level link to it lets Access perform the normal login flow without exposing an API redirect to AJAX callers. Set `ACCESS_TEAM_DOMAIN` (for example `team.cloudflareaccess.com`) and `ACCESS_AUD` as Worker variables/secrets; these values are intentionally not included here. For example, an operator may use `npx wrangler secret put ACCESS_TEAM_DOMAIN` and `npx wrangler secret put ACCESS_AUD`, or configure them in the dashboard. `src/worker/index.ts` obtains the Access JWKS from `/cdn-cgi/access/certs` and verifies the JWT signature, issuer, and audience before using its email claim. A raw email header is not trusted in production. Use `npx wrangler types` when binding or variable changes need regenerated Worker types.

   During an Access audience migration, use a comma-delimited `ACCESS_AUD` value containing both the old and new audience in the form `old-audience,new-audience`. Remove the old value after all old sessions have expired. Never place real domains, audiences, credentials, or tokens in this repository.
