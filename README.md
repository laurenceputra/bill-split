...
When upgrading a deployment created from an earlier notification-enabled
revision, regenerate `wrangler.deploy.toml` from the current example, remove
all old Queue and VAPID entries, re-encode the complete file, and replace the
`WRANGLER_DEPLOY_TOML_BASE64` build secret with the new encoding. Do not print,
log, or otherwise expose the decoded config or any secret values.
...
5. Apply and verify all remote migrations before deploying:
   ...
   ```sh
    ...
    Notification migrations `0027_notifications.sql`,
    `0028_notification_maintenance_indexes.sql`, and
    `0029_notification_fanout_pagination.sql` were removed because they were
    never deployed. Do not restore or apply them.
...
### Removing partial notification provisioning

Deploy the current code with the regenerated config first. After the deployment
is healthy, detach and delete any notification Queue producer/consumer
bindings and notification queues that were provisioned by an earlier revision.
Remove obsolete Worker variables `VAPID_PUBLIC_KEY` and `VAPID_CONTACT`, then
remove the obsolete Worker secrets `VAPID_PRIVATE_KEY` and
`PUSH_SUBSCRIPTION_ENCRYPTION_KEY` from the dashboard or with Wrangler. Do not
include secret values in commands, logs, or documentation; only remove the
named resources, variables, and secrets.
