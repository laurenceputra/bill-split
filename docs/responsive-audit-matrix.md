...
| Group overview | The single balances anchor precedes transparent ledger rows; schedules, people, and tools disclose below | Same DOM order with touch-sized controls; Record credit and compact spending stay together after ledger content without nested cards | Single-column financial reading order; balance entries and recent activity use dividers | Verify no premature desktop reflow or extra painted ancestor | The balances anchor leads a purposeful main/secondary layout; tools remain grouped after people | Two-column layout keeps money and status dominant while sections stay borderless |
| Group overview / balances loaded | Loaded balance collection is a transparent divider-separated ledger | Balance labels and amounts retain financial reading order without nested cards | Balance entries remain single-column and readable | Verify the collection stays single-column through 895px | Two-column balance arrangement is intentional; divider rules end by visual row | Two-column balance arrangement is intentional; divider rules end by visual row |
| Group overview / balance loading, error, or offline variants | No dedicated browser audit fixture currently covers these group-balance states | No dedicated browser audit fixture currently covers these group-balance states | No dedicated browser audit fixture currently covers these group-balance states | No dedicated browser audit fixture currently covers these group-balance states | No dedicated browser audit fixture currently covers these group-balance states | No dedicated browser audit fixture currently covers these group-balance states |
| Group overview / compact insight, populated | Compact spending follows the balance ledger and stays transparent | Compact spending remains a flattened secondary module after primary actions | Verify compact currency rows do not become nested cards | Verify no painted outer insight ancestor at the breakpoint | Compact spending shares the tools row without disrupting financial priority | Compact spending remains secondary to the group ledger and actions |
