...
describe('frontend API errors and cache fallback', () => {
  it('defines trusted offline cache contracts for global and contextual Add routes', () => {
    ...
    expect(provisionalAddRoute('/groups/group-1/add/')).toEqual({ scope: 'group', groupId: 'group-1' });
    expect(provisionalAddRoute('/groups/group-1/manage')).toBeUndefined();
  });

  it('passes an explicit bounded category trend range to the insights API', async () => {
    const fetchSpy = vi.fn(async (_input: RequestInfo | URL) => json({ scope: 'global', trendFrom: '2025-08-01', trendTo: '2026-01-31', categoryTrends: [] }));
    vi.stubGlobal('fetch', fetchSpy);
    await getSpendingInsightTrends(undefined, { trendFrom: '2025-08-01', trendTo: '2026-01-31' });
    expect(String(fetchSpy.mock.calls[0]?.[0])).toContain('/api/spending-insights?view=trends&trendFrom=2025-08-01&trendTo=2026-01-31');
  });

  it('does not opt summary-only insight requests into category trends', async () => {
    const fetchSpy = vi.fn(async (_input: RequestInfo | URL) => json({ scope: 'group', summaries: [] }));
    vi.stubGlobal('fetch', fetchSpy);
    await getSpendingInsightSummary('group-a');
    expect(String(fetchSpy.mock.calls[0]?.[0])).toBe('/api/spending-insights?group=group-a&view=summary');
  });
  ...
  it('keeps the legacy minimal group creation request compatible', async () => {
    const calls: Array<{ path: string; body?: string }> = [];
    vi.stubGlobal('fetch', vi.fn(async (request: RequestInfo | URL, init?: RequestInit) => {
      calls.push({ path: String(request), body: init?.body as string | undefined });
      return json({ group: { id: 'group-a', name: 'Shared', currency: 'USD', kind: 'named', createdAt: '', updatedAt: '' } });
    }));
    await createGroup({ name: 'Shared', currency: 'USD' });
    expect(calls).toEqual([{ path: '/api/groups', body: JSON.stringify({ name: 'Shared', currency: 'USD' }) }]);
  });

  it('requests named-to-peer conversion without a payload', async () => {
    await establishAuthenticatedMutationSession('clerk-peer-conversion');
    let request: { path: string; method?: string; body?: BodyInit | null } | undefined;
    vi.stubGlobal('fetch', vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      request = { path: String(input), method: init?.method, body: init?.body };
      return json({ group: { id: 'group-a', name: 'Shared', currency: 'USD', kind: 'peer', createdAt: '', updatedAt: '' } });
    }));

    await convertNamedToPeerGroup('group-a');
    expect(request).toEqual({ path: '/api/groups/group-a/convert-to-peer', method: 'POST', body: undefined });
  });
