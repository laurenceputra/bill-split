...
export const resourceKeys = Object.freeze({
  ...
  auditEntity: (userId: string, groupId: string, entityType: string, entityId: string) => resourceKey('audit-entity', `${userId}:${groupId}:${entityType}:${entityId}`),
...
export const invalidateForMutation = {
  ...
  profileChanged: async (userId: string, generation?: number, profile?: { personId: string; name: string; profileRevision?: number; updatedAt?: string }) => {
    ...
    for (const prefix of ['groups:', 'group:', 'members:', 'expenses:', 'transactions:', 'balances:', 'settlements:', 'scheduled-expenses:', 'scheduled-expense:', 'activity:', 'audit:', 'audit-entity:', 'expense-detail:', 'settlement-detail:']) invalidateResourcePrefix(prefix, userId);
