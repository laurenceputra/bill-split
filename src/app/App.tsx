...
import { browserTimezone, formatScheduleDate, previewScheduleDates, scheduleContinuationText, scheduleSummary, timezoneLabel, timezoneOptions, weekdayLabels } from './scheduled-expense';
       ...
       const splits = allocationSplits(selected, method, preview, allocationValues, existingSplitMetadata);
        if (recurring) {
         ...
         if (endDate && endDate < date) throw new Error('End date must not precede the start date.');
          ...
          const scheduleInput: ScheduledExpenseInput = { description: description.trim(), amount_minor: cents, currency, category: category.trim() || null, start_date: date, end_date: endDate || null, frequency, interval: scheduleInterval, weekdays: frequency === 'weekly' ? weekdays : [], timezone: timezone.trim() || 'UTC', payers, splits, version, client_operation_id: scheduledExpenseId ? undefined : operation };
...
function Activity() {
  ...
  const me = useResource(resourceKeys.identity(), '', (signal) => getMe({ signal }), RESOURCE_FRESHNESS.expenses, hydrateIdentity);
  const groupsResource = useResource<{ groups: Group[] }>(resourceKeys.groups(me.data?.id || 'pending'), me.data?.id, (signal) => getGroups(signal), RESOURCE_FRESHNESS.groups, me.data?.id ? () => hydrateGroups(me.data!.id) : undefined);
  const activity = useResource<{ activity: ActivityItem[] }>(resourceKeys.activity(me.data?.id || 'pending', selectedGroupId || 'all'), me.data?.id, (signal) => getActivity(selectedGroupId, signal), RESOURCE_FRESHNESS.activity, me.data?.id ? () => hydrateActivity(me.data!.id, selectedGroupId || 'all') : undefined);
  ...
  const itemRow = (item: ActivityItem) => {
    const content = <>{row(item)}{item.amountMinor != null && item.currency ? <Money amountMinor={item.amountMinor} currency={item.currency} /> : null}</>;
       const path = activityDetailPath(item.groupId, item);
