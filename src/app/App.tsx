...
import { browserTimezone, formatScheduleDate, previewScheduleDates, scheduleContinuationText, scheduleSummary, weekdayLabels } from './scheduled-expense';
  ...
  const memberLabel = (personId: string) => personId === currentPersonId ? 'You' : nameOf(members, personId);
   ...
   return <Layout>
     <Link to="/" className="back">← Groups</Link>
       <div className="page-title"><div><p className="eyebrow">{group.currency} group</p><h1>{group.name}</h1></div><div className="expense-heading__actions"><Link className="button" to={`/groups/${id}/expense/new`}>+ Add expense</Link></div></div>
...
function OneTimeOnlyDetails({ category, notes, onCategoryChange, onNotesChange }: { category: string; notes: string; onCategoryChange: (value: string) => void; onNotesChange: (value: string) => void }) {
  return <fieldset className="one-time-only-details" disabled aria-describedby="one-time-only-details-help"><legend>One-time details</legend><p id="one-time-only-details-help" className="muted">Category and notes are saved only for one-time expenses. Your entries are kept and return if Repeat is unchecked.</p><Field label="Category (optional)" className="field--compact"><input className="category" value={category} onChange={(event) => onCategoryChange(event.target.value)} /></Field><Field label="Notes (optional)" className="field--compact"><textarea className="notes" rows={3} value={notes} onChange={(event) => onNotesChange(event.target.value)} /></Field></fieldset>;
}
...
function ExpenseForm() {
  ...
  const payerSummary = payerRows.length === 1 ? `Paid by ${isYou(payerRows[0].personId) ? 'You' : nameOf(members, payerRows[0].personId)}` : payerRows.length ? `Paid by ${payerRows.length} people` : 'Choose who paid';
   const payerSummaryDetail = payerRows.length === 1 ? (payerIsFullTotal ? 'Entire total' : 'Amount needs review') : payerRows.length ? 'Configure exact amounts' : 'Choose a payer';
    ...
    const schedulePreview = recurring ? (() => { try { return previewScheduleDates(scheduleDraft, localDateForTimeZone(new Date(), timezone.trim() || 'UTC'), 3); } catch { return previewScheduleDates(scheduleDraft, today(), 3); } })() : [];
...
function PrivateRoutes() {
  ...
  return <Routes key={identityEpoch}><Route path="/" element={<Home />} /><Route path="/settings" element={<Settings />} /><Route path="/groups/:id" element={<GroupPage />} /><Route path="/groups/:id/expense/new" element={<ExpenseForm />} /><Route path="/groups/:id/expense/:expenseId" element={<ExpenseForm />} /><Route path="/groups/:id/scheduled-expense/new" element={<LegacyScheduledExpenseRedirect />} /><Route path="/groups/:id/scheduled-expense/:scheduledExpenseId" element={<ExpenseForm />} /><Route path="/groups/:id/expenses/:expenseId" element={<ExpenseDetail />} /><Route path="/expenses/:expenseId" element={<ExpenseDetail />} /><Route path="/groups/:id/settle" element={<Settle />} /><Route path="/groups/:id/activity" element={<Activity />} /><Route path="*" element={<Navigate to="/" replace />} /></Routes>;
}
