...
import { categoryTrendStatus, insightBarWidth, insightComparisonDateRange, insightCurrencySet, insightMonthLabel, insightQuery, insightTrendDateRange, insightTrendMonths, readInsightFilters, topCategoryTrends, validInsightRange, type InsightFilters } from './spending-insights';
...
function SpendingInsightsPage({ groupId, groupName }: { groupId?: string; groupName?: string }) {
  ...
  const me = useResource(resourceKeys.identity(), '', (signal) => getMe({ signal }), RESOURCE_FRESHNESS.expenses, hydrateIdentity);
  const userId = me.data?.id || 'pending';
  const summaryKey = `${groupId || 'all'}:summary:${insightQuery(filters).toString()}:${comparison.from || ''}:${comparison.to || ''}`;
  const trendKey = `${groupId || 'all'}:trends:${trendRange.trendFrom}:${trendRange.trendTo}:${filters.currency || ''}`;
  const emptySummary: SpendingInsightSummaryResponse = { scope: groupId ? 'group' : 'global', summaries: [] };
  const summary = useResource<SpendingInsightSummaryResponse>(resourceKeys.insights(userId, summaryKey), me.data?.id, (signal) => customValid ? getSpendingInsightSummary(groupId, { from: filters.from, to: filters.to, currency: filters.currency, comparisonFrom: comparison.from, comparisonTo: comparison.to }, signal) : Promise.resolve(emptySummary), RESOURCE_FRESHNESS.insights);
  const trends = useResource<SpendingInsightTrends>(resourceKeys.insights(userId, trendKey), me.data?.id, (signal) => getSpendingInsightTrends(groupId, { ...trendRange, currency: filters.currency }, signal), RESOURCE_FRESHNESS.insights);
  ...
  const currencies = insightCurrencySet(summary.data, trends.data);
  ...
  const summaryCurrencies = [...new Set([...(summary.data?.summaries || []).map((item) => item.currency), ...(summary.data?.previous?.summaries || []).map((item) => item.currency)])].sort();
  if (!online && (summary.data === undefined || trends.data === undefined)) return <section className="insights-page" aria-labelledby="insights-heading"><h2 id="insights-heading">Spending insights</h2><p className="muted">Refresh is unavailable offline.</p><section aria-labelledby="insight-summary-heading"><h3 id="insight-summary-heading">{groupId ? 'How much did this group spend?' : 'How much did I spend?'}</h3>{summary.data ? <>{<p className="cache-status" role="status">Refresh unavailable offline; showing cached selected-period summary.</p>}{summaryCurrencies.length ? <div className="insight-summary-grid" aria-label="Selected-period spending summary">{summaryCurrencies.map((currencyValue) => <InsightPrimarySummary key={currencyValue} summary={summary.data!.summaries.find((item) => item.currency === currencyValue) || zeroInsightSummary(currencyValue)} data={summary.data!} previous={summary.data!.previous?.summaries.find((item) => item.currency === currencyValue)} />)}</div> : <Empty>No counted expenses in this period.</Empty>}</> : <InsightUnavailableOffline label="Selected-period summary" />}</section><section aria-labelledby="insight-change-heading"><h3 id="insight-change-heading">Where is spending changing?</h3>{trends.data ? <>{<p className="cache-status" role="status">Refresh unavailable offline; showing cached six-month trends.</p>}{currencies.map((currencyValue, index) => <SimplifiedCategoryTrendModule key={currencyValue} data={trends.data!} currencyValue={currencyValue} index={index} />)}</> : <InsightUnavailableOffline label="Six-month trends" />}</section></section>;
  return <section className="insights-page" aria-labelledby="insights-heading"><div className="section-title"><div><h2 id="insights-heading">Spending insights</h2><p className="muted">{groupId ? `${groupName || 'Selected group'} · ${periodLabel}` : `All groups · ${periodLabel}`}{filters.from && filters.to ? ` · ${filters.from} to ${filters.to}` : ''}</p></div></div><div className="insight-controls" aria-label="Spending insight filters"><Field label="Period"><select value={filters.period} onChange={(event) => selectPeriod(event.target.value as InsightFilters['period'])}><option value="month">This month to date</option><option value="last-month">Last month</option><option value="year">This year to date</option><option value="all">All time</option><option value="custom">Custom range</option></select></Field>{filters.period === 'custom' ? <><Field label="From" errorId="insights-custom-from-error" error={customDraft.from !== '' && !validInsightRange(customDraft.from, customDraft.from) ? 'Enter a real start date.' : undefined}><input type="date" value={customDraft.from} aria-invalid={customDraft.from !== '' && !validInsightRange(customDraft.from, customDraft.from)} aria-describedby={customDraft.from !== '' && !validInsightRange(customDraft.from, customDraft.from) ? 'insights-custom-from-error' : undefined} onChange={(event) => { setCustomDraft((current) => ({ ...current, from: event.target.value })); setCustomError(''); }} /></Field><Field label="To" errorId="insights-custom-to-error" error={customDraft.to !== '' && !validInsightRange(customDraft.to, customDraft.to) ? 'Enter a real end date.' : undefined}><input type="date" value={customDraft.to} aria-invalid={customDraft.to !== '' && !validInsightRange(customDraft.to, customDraft.to)} aria-describedby={customDraft.to !== '' && !validInsightRange(customDraft.to, customDraft.to) ? 'insights-custom-to-error' : undefined} onChange={(event) => { setCustomDraft((current) => ({ ...current, to: event.target.value })); setCustomError(''); }} /></Field><Button type="button" aria-describedby={customError ? 'insights-custom-range-error' : undefined} onClick={applyCustom}>Apply range</Button>{customError ? <p id="insights-custom-range-error" className="error" role="alert">{customError}</p> : null}</> : null}{currencies.length > 1 || selectedCurrency ? <Field label="Currency"><select value={selectedCurrency || ''} onChange={(event) => { const next = new URLSearchParams(searchParams); if (event.target.value) next.set('currency', event.target.value); else next.delete('currency'); updateUrl(next); }}><option value="">All currencies</option>{currencies.map((currencyValue) => <option key={currencyValue} value={currencyValue}>{currencyValue}</option>)}</select></Field> : null}</div>{!online ? <><ConnectionBanner detail="Refresh is unavailable offline; cached insight data may be shown." /><p className="muted" role="status">Refresh is unavailable offline; cached data may be shown. New server aggregates require a connection.</p></> : null}<section aria-labelledby="insight-summary-heading"><div className="section-title"><h3 id="insight-summary-heading">{groupId ? 'How much did this group spend?' : 'How much did I spend?'}</h3><span className="muted">Selected period</span></div>{filters.period === 'custom' && !customValid ? <p className="muted" role="status">Choose two valid dates to load the selected-period summary. Six-month trends remain available.</p> : summary.data === undefined ? summary.error ? <ErrorBox error={summary.error} onRetry={online ? summaryRetry : undefined} id="insights-summary-error" /> : <Loading /> : <>{summary.error || summary.stale || summary.offline ? <ResourceNotice resource={summary} label="selected-period summary" retry={online ? summaryRetry : undefined} /> : null}{summaryCurrencies.length ? <div className="insight-summary-grid" aria-label="Selected-period spending summary">{summaryCurrencies.filter((currencyValue) => !selectedCurrency || currencyValue === selectedCurrency).map((currencyValue) => <InsightPrimarySummary key={currencyValue} summary={summary.data!.summaries.find((item) => item.currency === currencyValue) || zeroInsightSummary(currencyValue)} data={summary.data!} previous={summary.data!.previous?.summaries.find((item) => item.currency === currencyValue)} />)}</div> : <Empty>No counted expenses in this period. Settlements, scheduled expenses, deleted expenses, and pending offline expenses are excluded.</Empty>}</>}</section><section aria-labelledby="insight-change-heading"><div className="section-title"><h3 id="insight-change-heading">Where is spending changing?</h3><span className="muted">Independent six-month view</span></div>{trends.data === undefined ? trends.error ? <ErrorBox error={trends.error} onRetry={online ? trendRetry : undefined} id="insights-trends-error" /> : <Loading /> : <>{trends.error || trends.stale || trends.offline ? <ResourceNotice resource={trends} label="six-month trends" retry={online ? trendRetry : undefined} /> : null}{currencies.filter((currencyValue) => !selectedCurrency || currencyValue === selectedCurrency).map((currencyValue, index) => <SimplifiedCategoryTrendModule key={currencyValue} data={trends.data!} currencyValue={currencyValue} index={index} />)}{!currencies.length ? <Empty>No counted category spending in the latest six months.</Empty> : null}</>}</section></section>;
}
