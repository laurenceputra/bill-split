...
import { AuthLoadingShell, AvatarStack, Button, EmptyState, Field, InstallAction, Layout, Modal, Money, PublicShell, Skeleton, SplitTransactionControl, Status, Surface, connectionStatusLabel, useAuthLifecycle, useConnectionState, useOnlineStatus } from './ui';
...
function GroupOverview() {
  ...
  return <Layout><Link to="/" className="back">← Groups</Link><div className="page-title"><div><p className="eyebrow">{group.kind === 'peer' ? 'Friend ledger' : group.kind === undefined && group.memberCount === 2 ? 'Friend ledger (cached)' : `${group.currency} group`}</p><h1>{displayName}</h1></div><div className="expense-heading__actions"><SplitTransactionControl groupId={id} online={online && !offline} /><Link className="button button--secondary" to={`/groups/${id}/settle`}>Settle up</Link></div></div>{offline ? <ConnectionBanner detail="showing cached group data. New expenses can be captured; history, schedules, and management need a connection." /> : null}{me.error ? <CachedIdentityNotice resource={me} id="group-identity-error" /> : null}{groupResource.error ? <ResourceNotice resource={groupResource} label="group" retry={retryFor(resourceKeys.group(userId, id), me.data?.id)} /> : null}{refreshing ? <p className="cache-status" role="status">Refreshing group data…</p> : null}
    ...
    <section aria-labelledby="people-summary-heading"><div className="section-title"><h2 id="people-summary-heading">People</h2><span className="muted">{members.length} {members.length === 1 ? 'person' : 'people'}</span></div><p className="people-summary-compact">{members.slice(0, 4).map((member) => member.personId === currentPersonId ? 'You' : member.name).join(', ')}{members.length > 4 ? ` +${members.length - 4} more` : ''}</p><Link className="inline-action" to={`/groups/${id}/manage#people`}>Manage people</Link></section>
       <section className="group-overview-tools" aria-labelledby="group-tools-heading"><div className="section-title"><div><h2 id="group-tools-heading">More group tools</h2><span className="muted">Accounting and history</span></div></div><div className="group-overview-actions actions"><Link className="button button--secondary" to={`/groups/${id}/credit/new`}>Record credit</Link><CompactInsights groupId={id} userId={me.data?.id} title="Group spending" /></div><nav className="actions group-tools" aria-label="Group tools"><Link className="button button--secondary" to={`/activity?group=${encodeURIComponent(id)}&view=changes`}>Group History</Link><Link className="button button--secondary" to={`/groups/${id}/manage#settings`}>Group settings</Link></nav></section>
  </Layout>;
}
