...
import { createCredit as createCreditRequest, deleteCredit, getCreditDetails, getExpensePage, restoreCredit, updateCredit, type ExpensePage } from './api';
...
import { creditDetailPath, expenseDetailPath, getNavigationContext, settlementDetailPath, transactionActivityPath } from './navigation';
...
function CompactInsights({ groupId, userId, title = 'Spending', global = false }: { groupId?: string; userId?: string; title?: string; global?: boolean }) {
  ...
  if (resource.data === undefined) {
    if (!online) return <Surface className="insights-compact"><div className="section-title"><h2>{title}</h2></div><p className="muted" role="status">Refresh is unavailable offline; cached spending data may be shown when available.</p></Surface>;
  ...
  const addFriendRequested = searchParams.get('friend') === '1';
   ...
   if (coldHomeLoading) return <Layout><HomeLoadingPlaceholder /></Layout>;
    return <Layout>
      ...
      {groupsResource.data === undefined && !me.error ? <ResourceNotice resource={groupsResource} label="groups" retry={retryFor(resourceKeys.groups(me.data?.id || 'pending'), me.data?.id)} /> : groupsResource.data !== undefined ? <><ResourceNotice resource={groupsResource} label="groups" retry={retryFor(resourceKeys.groups(me.data?.id || 'pending'), me.data?.id)} />{groups.length ? <div className="cards">{groups.map((group) => <Link className="card" to={`/groups/${group.id}`} key={group.id}><strong className="card__name">{groupDisplayName(group)}</strong><div className="card__balances">{groupBalanceDisplays(group.balanceSummaries, group.currency).map((display, index) => display.kind === 'balance' ? <span className="card__balance" key={`${display.currency}-${index}`}><span className="card__balance-label">{display.label}</span><span className="card__balance-money"><Money amountMinor={display.amountMinor} currency={display.currency} tone={display.label === 'You are owed' ? 'positive' : 'debt'} /><small>{display.currency}</small></span></span> : <span className={`card__balance card__balance--${display.kind}`} key={`${display.label}-${index}`}><span className="card__balance-label">{display.label}</span><small>{display.currency}</small></span>)}</div></Link>)}</div> : <Empty>No groups yet. <Link to="/friends/new">Add a friend</Link> or <Link to="/groups/new">create a group</Link> to get started.</Empty>}</> : null}
  </Layout>;
}
...
function GroupOverview() {
  ...
  return <Layout><Link to="/" className="back">← Groups</Link><div className="page-title"><div><p className="eyebrow">{group.kind === 'peer' ? 'Friend ledger' : group.kind === undefined && group.memberCount === 2 ? 'Friend ledger (cached)' : `${group.currency} group`}</p><h1>{displayName}</h1></div><div className="expense-heading__actions"><Link className="button" to={`/groups/${id}/expense/new`}>+ Add expense</Link><Link className="button button--secondary" to={`/groups/${id}/settle`}>Settle up</Link></div></div>{offline ? <ConnectionBanner detail="showing cached group data. New expenses can be captured; history, schedules, and management need a connection." /> : null}{me.error ? <CachedIdentityNotice resource={me} id="group-identity-error" /> : null}{groupResource.error ? <ResourceNotice resource={groupResource} label="group" retry={retryFor(resourceKeys.group(userId, id), me.data?.id)} /> : null}{refreshing ? <p className="cache-status" role="status">Refreshing group data…</p> : null}
      <section aria-labelledby="balances-heading" className="compact-balances"><h2 id="balances-heading">Your balances</h2>{balancesResource.data !== undefined ? <ResourceNotice resource={balancesResource} label="balances" retry={retryFor(resourceKeys.balances(userId, id), me.data?.id)} /> : balancesResource.error ? <ErrorBox error={balancesResource.error} onRetry={online ? retryFor(resourceKeys.balances(userId, id), me.data?.id) : undefined} id="balances-error" /> : null}<div className="balance-cards">{balancesResource.data === undefined ? <p className="cache-status">Balances are unavailable until this group’s balance data is loaded.</p> : balanceDisplays.map((display, index) => <div className={`balance-card balance-card--${display.kind}`} key={`${display.currency}-${index}`}><span className="balance-card__currency">{display.currency}</span><strong>{display.label}</strong>{display.kind === 'balance' ? <Money amountMinor={display.amountMinor} currency={display.currency} tone={display.label === 'You are owed' ? 'positive' : 'debt'} /> : null}</div>)}</div><details className="balance-breakdown"><summary>View full group breakdown</summary><p className="muted">Net position = paid minus share, adjusted by settlements. Positive means owed; negative means owes.</p>{balancesResource.data === undefined ? <p className="cache-status">Balance details are unavailable.</p> : Object.keys(balances).length ? Object.entries(balances).map(([currencyKey, balance]) => <div key={currencyKey}><h3>{currencyKey}</h3><div className="list">{balance.raw.map((item) => <div className="row" key={`${currencyKey}-raw-${item.personId}`}><span>{item.personId === currentPersonId ? 'You' : item.name}</span><span>{balanceStatus(item.netMinor) === 'owed' ? 'is owed' : balanceStatus(item.netMinor) === 'owes' ? 'owes' : 'settled up'}</span>{item.netMinor === 0 ? null : <Money amountMinor={Math.abs(item.netMinor)} currency={currencyKey} tone={item.netMinor > 0 ? 'positive' : 'debt'} />}</div>)}</div><h4>Suggested transfers</h4>{balance.simplified.length ? <div className="list">{balance.simplified.map((item) => <div className="row" key={`${currencyKey}-${item.fromPersonId}-${item.toPersonId}`}><span>{item.fromPersonId === currentPersonId ? 'You' : item.fromName} owes {item.toPersonId === currentPersonId ? 'You' : item.toName}</span><Money amountMinor={item.amountMinor} currency={currencyKey} tone="debt" /></div>)}</div> : <p className="muted">Everyone is settled up.</p>}</div>) : <Empty>Everyone is settled up.</Empty>}</details></section>
        <div className="actions"><Link className="button button--secondary" to={`/groups/${id}/credit/new`}>Record credit</Link></div><CompactInsights groupId={id} userId={me.data?.id} title="Group spending" />
...
function CreditForm({ initialCredit }: { initialCredit?: Credit } = {}) {
  ...
  const [date, setDate] = useState(initialCredit?.date || today());
  const [note, setNote] = useState(initialCredit?.note || '');
  const [applications, setApplications] = useState<CreditApplicationDraft[]>(() => (initialCredit?.applications || []).map((item) => ({ id: operationId(), expenseId: item.expenseId, amount: moneyInput(item.amountMinor) })));
  const [allocations, setAllocations] = useState<CreditAllocationDraft[]>(() => initialCredit?.allocations?.length ? initialCredit.allocations.map((item) => ({ id: operationId(), personId: item.personId, allocationType: item.allocationType, amount: moneyInput(item.amountMinor) })) : [{ id: operationId(), personId: '', allocationType: 'recipient', amount: '' }, { id: operationId(), personId: '', allocationType: 'beneficiary', amount: '' }]);
  ...
  const updateAllocation = (rowId: string, patch: Partial<CreditAllocationDraft>) => setAllocations((current) => current.map((row) => row.id === rowId ? { ...row, ...patch } : row));
  const submit = async (event: FormEvent) => {
    event.preventDefault(); if (!online || busy || !group) return;
    setBusy(true); setError(undefined);
    try {
      const amountMinor = parseMoney(amount, group.currency);
      const validationError = validateCreditDraft(applications, allocations, mode);
      if (validationError) throw new Error(validationError);
      const input = { subtype, delivery_mode: mode, amount_minor: amountMinor, currency: group.currency, date, note: note || null,
        applications: applications.map((row) => ({ expense_id: row.expenseId, amount_minor: parseMoney(row.amount, group.currency) })),
        allocations: mode === 'direct_provider_offset' ? [] : allocations.filter((row) => row.personId).map((row) => ({ person_id: row.personId, allocation_type: row.allocationType, amount_minor: parseMoney(row.amount, group.currency) })), client_operation_id: operation };
      if (initialCredit) await updateCredit(initialCredit.id, { ...input, version: initialCredit.version }); else await createCreditRequest(id, input);
...
function CreditEditForm() {
  const { creditId = '' } = useParams(); const me = useResource(resourceKeys.identity(), '', (signal) => getMe({ signal }), RESOURCE_FRESHNESS.expenses, hydrateIdentity); const details = useResource<{ credit: Credit }>(resourceKeys.creditDetail(me.data?.id || 'pending', creditId), me.data?.id, (signal) => getCreditDetails(creditId, signal), RESOURCE_FRESHNESS.expenseDetail);
...
function CreditDetail() {
  ...
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState<unknown>();
  if ((details.error || me.error) && !credit) return <Layout><ErrorBox error={details.error || me.error} onRetry={retryFor(details.error ? resourceKeys.creditDetail(me.data?.id || 'pending', creditId) : resourceKeys.identity(), me.data?.id)} id="credit-detail-error" /><Link className="back" to="/">← Home</Link></Layout>;
  if (!credit) return <Layout><Loading /></Layout>;
  const remove = async () => { if (!online || busy || !confirm('Delete this credit?')) return; setBusy(true); setError(undefined); try { await deleteCredit(credit.id, credit.version); await invalidateForMutation.creditChanged(credit.groupId, me.data?.id, credit.id, captureSessionGeneration()); nav(`/groups/${credit.groupId}`); } catch (cause) { setError(cause); } finally { setBusy(false); } };
  const restore = async () => { if (!online || busy) return; setBusy(true); setError(undefined); try { await restoreCredit(credit.id, credit.version); await invalidateForMutation.creditChanged(credit.groupId, me.data?.id, credit.id, captureSessionGeneration()); } catch (cause) { setError(cause); } finally { setBusy(false); } };
  return <Layout><Link to={`/groups/${credit.groupId}`} className="back">← Group</Link><div className="page-title"><div><p className="eyebrow">{credit.date} · {credit.subtype}</p><h1>{credit.deliveryMode === 'direct_provider_offset' ? 'Direct-provider credit' : 'Member reimbursement'}</h1></div><Money amountMinor={credit.amountMinor} currency={credit.currency} size="large" /></div>{!online ? <ConnectionBanner detail="Credit changes require a connection." /> : null}{credit.deletedAt ? <Surface><strong>Deleted credit</strong><p className="muted">This credit can be restored within 30 days.</p><Button disabled={!online || busy} onClick={() => void restore()}>{busy ? 'Restoring…' : 'Restore credit'}</Button></Surface> : <Surface><p className="muted">Confirmed credit · version {credit.version}</p><h2>Applications</h2><div className="list">{credit.applications.length ? credit.applications.map((application) => <div className="row" key={application.expenseId}><span>{application.expenseDescription || application.expenseId}</span><span>{moneyInput(application.amountMinor)} {credit.currency}</span></div>) : <p className="muted">Standalone credit</p>}</div><h2>Allocations</h2><div className="list">{credit.allocations.map((allocation) => <div className="row" key={`${allocation.allocationType}-${allocation.personId}`}><span>{allocation.allocationType} · {allocation.personId}</span><span>{moneyInput(allocation.amountMinor)} {credit.currency}</span></div>)}</div><div className="actions"><Link className="button button--secondary" to={`/groups/${credit.groupId}/credit/${credit.id}/edit`}>Edit credit</Link><Button type="button" variant="danger" disabled={!online || busy} onClick={() => void remove()}>{busy ? 'Deleting…' : 'Delete credit'}</Button></div></Surface>}{error ? <ErrorBox error={error} id="credit-detail-action-error" /> : null}<Surface><h2>History</h2>{details.data?.history.length ? <div className="list">{details.data.history.map((item) => <div className="row" key={item.id}><span>Revision {item.revision}</span><small>{item.createdAt}</small></div>)}</div> : <p className="muted">No history available.</p>}</Surface></Layout>;
}
...
function HistoryTransactions({ groupId, groups }: { groupId?: string; groups: Group[] }) {
  ...
  const clearFilters = () => setSearchParams(writeTransactionFilters(searchParams, {}));
   return <section className="history-panel" aria-labelledby="history-transactions-heading">
     <label className="history-kind-shortcut">Transaction kind <select value={draftFilters.kind || ''} disabled={offline} onChange={(event) => setDraftFilters((current) => ({ ...current, kind: (event.target.value || undefined) as TransactionFilters['kind'], category: event.target.value === 'credit' || event.target.value === 'settlement' ? undefined : current.category }))}><option value="">All transactions</option><option value="expense">Expenses</option><option value="settlement">Settlements</option><option value="credit">Credits</option></select></label>
    ...
    <TransactionFilterDisclosure filterKey={filterSignature} filterCount={filterCount} offline={offline}><form className="transaction-filters" onSubmit={applyFilters}><Field label="Search"><input type="search" value={draftFilters.q || ''} disabled={offline} onChange={(event) => setDraftFilters((current) => ({ ...current, q: event.target.value || undefined }))} /></Field><Field label="Kind"><select value={draftFilters.kind || ''} disabled={offline} onChange={(event) => setDraftFilters((current) => ({ ...current, kind: (event.target.value || undefined) as TransactionFilters['kind'], category: event.target.value === 'settlement' ? undefined : current.category }))}><option value="">All transaction types</option><option value="expense">Expenses</option><option value="settlement">Settlements</option></select></Field>{groupId ? <Field label="Person"><select value={draftFilters.person || ''} disabled={offline} onChange={(event) => setDraftFilters((current) => ({ ...current, person: event.target.value || undefined }))}><option value="">All people</option>{members.map((member) => <option key={member.personId} value={member.personId}>{member.name}</option>)}</select></Field> : null}<Field label="Category"><select value={draftFilters.category || ''} disabled={offline || draftFilters.kind === 'settlement'} onChange={(event) => setDraftFilters((current) => ({ ...current, category: event.target.value || undefined }))}><option value="">All categories</option>{categoryChoices.map((category) => <option key={category} value={category}>{category}</option>)}</select></Field><Field label="From date"><input type="date" value={draftFilters.from || ''} disabled={offline} onChange={(event) => setDraftFilters((current) => ({ ...current, from: event.target.value || undefined }))} /></Field><Field label="To date"><input type="date" value={draftFilters.to || ''} disabled={offline} onChange={(event) => setDraftFilters((current) => ({ ...current, to: event.target.value || undefined }))} /></Field><Field label="Currency"><select value={draftFilters.currency || ''} disabled={offline} onChange={(event) => setDraftFilters((current) => ({ ...current, currency: event.target.value as Currency || undefined }))}><option value="">All currencies</option>{currencyOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></Field><div className="actions"><Button type="submit" disabled={offline}>Apply</Button>{filterCount ? <Button type="button" variant="secondary" disabled={offline} onClick={clearFilters}>Clear</Button> : null}</div></form></TransactionFilterDisclosure>
      {filteredOfflineUnavailable ? <p className="cache-status" role="status">Filtered transaction results aren’t cached on this device. Reconnect to search history.</p> : <ResourceNotice resource={transactionsResource} label="transactions" retry={retryFor(resourceKeys.transactions(userId, groupId || 'all', filterSignature), me.data?.id)} />}{pending.length ? <section className="pending-transactions" aria-labelledby="pending-transactions-heading"><h3 id="pending-transactions-heading">Waiting to sync · {pending.length}</h3><div className="list">{pending.map((item) => <PendingExpenseRow key={item.clientOperationId} item={item} groupName={!groupId ? groupNames.get(item.groupId) : undefined} />)}</div></section> : null}{pageState && rows.length ? <div className="list transaction-list">{rows.map((transaction) => <TransactionRow key={transactionKey(transaction)} groupId={transaction.groupId} transaction={transaction} currentPersonId={me.data?.personId} showNotes />)}</div> : pageState && !pending.length ? <Empty>No transactions match the current filters.</Empty> : null}{pageState?.cursor ? <Button type="button" variant="secondary" disabled={loadingMore || offline} onClick={() => void loadMore()}>{loadingMore ? 'Loading…' : 'Load more transactions'}</Button> : null}{pageState?.pageError ? <ErrorBox error={pageState.pageError} id="history-transactions-page-error" /> : null}
  </section>;
}
...
function HistoryChanges({ groupId }: { groupId?: string }) {
  ...
  const loadMore = async () => { if (!cursor || loadingMore || !online) return; const request = pageScope.current.begin(scopeKey, cursor); loadingCursorRef.current = request.cursor; setLoadingMore(true); setPageError(undefined); try { const page = await getActivityPage(groupId, { cursor: request.cursor }, request.signal); if (!pageScope.current.isCurrent(request) || scopeKeyRef.current !== request.key || cursorRef.current !== request.cursor) return; cursorRef.current = page.nextCursor; setItems((current) => appendUniquePage(current, page.activity, (item) => `${item.type}:${item.id}`)); setCursor(page.nextCursor); } catch (cause) { if (pageScope.current.isCurrent(request) && scopeKeyRef.current === request.key && cursorRef.current === request.cursor && loadingCursorRef.current === request.cursor && !(cause instanceof DOMException && cause.name === 'AbortError')) setPageError(cause); } finally { if (pageScope.current.isCurrent(request) && scopeKeyRef.current === request.key && (cursorRef.current === request.cursor || loadingCursorRef.current === request.cursor)) { loadingCursorRef.current = undefined; setLoadingMore(false); } } };
   const typeLabel = (type: ActivityItem['type']) => ({ expense: 'Expense', settlement: 'Settlement', credit: 'Credit', expense_revision: 'Expense edited', settlement_revision: 'Settlement edited', credit_revision: 'Credit edited', expense_deleted: 'Expense deleted', settlement_deleted: 'Settlement deleted', credit_deleted: 'Credit deleted' })[type];
...
function PrivateRoutes() {
  ...
  return <Routes key={identityEpoch}><Route path="/" element={<Home />} /><Route path="/friends/new" element={<FriendCreationPage />} /><Route path="/groups/new" element={<GroupCreationPage />} /><Route path="/settings" element={<Settings />} /><Route path="/activity" element={<History />} /><Route path="/expense/new" element={<ExpenseForm />} /><Route path="/groups/:id" element={<GroupOverview />} /><Route path="/groups/:id/transactions" element={<LegacyTransactionRedirect />} /><Route path="/groups/:id/manage" element={<GroupManagementPage />} /><Route path="/groups/:id/expense/new" element={<ExpenseForm />} /><Route path="/groups/:id/credit/new" element={<CreditForm />} /><Route path="/groups/:id/credit/:creditId/edit" element={<CreditEditForm />} /><Route path="/groups/:id/credits/:creditId" element={<CreditDetail />} /><Route path="/groups/:id/expense/:expenseId" element={<ExpenseForm />} /><Route path="/groups/:id/scheduled-expense/new" element={<LegacyScheduledExpenseRedirect />} /><Route path="/groups/:id/scheduled-expense/:scheduledExpenseId" element={<ExpenseForm />} /><Route path="/groups/:id/expenses/:expenseId" element={<ExpenseDetail />} /><Route path="/expenses/:expenseId" element={<ExpenseDetail />} /><Route path="/groups/:id/settle" element={<Settle />} /><Route path="/groups/:id/settlements/:settlementId" element={<SettlementDetail />} /><Route path="/groups/:id/activity" element={<LegacyActivityRedirect />} /><Route path="*" element={<Navigate to="/" replace />} /></Routes>;
}
