...
function TransactionRow({ groupId, transaction, showNotes = false }: { groupId: string; transaction: Transaction; showNotes?: boolean }) {
  ...
  const content = <><span><strong>{transactionTypeLabel(transaction)} · {transactionTitle(transaction)}</strong><small>{transactionDate(transaction)}{transactionPeople(transaction) ? ` · ${transactionPeople(transaction)}` : ''}</small>{category ? <small className="transaction-row__category">Category: {category}</small> : null}{note ? <small className="transaction-row__notes">Note: {note}</small> : null}</span><Money amountMinor={transaction.amountMinor} currency={transaction.currency} tone={transaction.kind === 'settlement' ? 'positive' : undefined} /></>;
...
function TransactionHistoryPage() {
  ...
  useEffect(() => {
    if (searchParams.get('category') && filters.kind === 'settlement') setSearchParams(writeTransactionFilters(searchParams, filters), { replace: true });
