...
import type { Activity as ActivityItem, AuditDisclosureEvent, Balances, Currency, Expense, Group, GroupInvitation, GroupMember, GroupResponse, GroupSplitDefault, HistoricalParticipant, RecurrenceFrequency, ScheduledExpense, ScheduledExpenseStatus, Settlement, SpendingInsights, SplitMethod, Transaction, Weekday } from '../shared/types';
...
import { categoriesForCurrency, insightBarWidth, insightCurrencyChoices, insightQuery, insightSentences, insightTransactionPath, readInsightFilters, summaryForCurrency, validInsightRange, type InsightFilters } from './spending-insights';
...
const nameOf = (members: GroupMember[], id: string) => members.find((member) => member.personId === id)?.name || 'Removed participant';
...
function InsightCurrencyMetrics({ summary, personal = false }: { summary: SpendingInsights['summaries'][number]; personal?: boolean }) {
  return <div className="insight-metric"><span className="insight-metric__currency">{summary.currency} · {personal ? 'Your allocated spending' : 'Group spending'}</span><strong><Money amountMinor={personal ? summary.allocatedSpendMinor : summary.groupSpendMinor} currency={summary.currency} /></strong><span className="muted">Your share <Money amountMinor={summary.yourShareMinor} currency={summary.currency} /></span><small>{summary.expenseCount} counted expense{summary.expenseCount === 1 ? '' : 's'}</small></div>;
}
...
function CompactInsights({ groupId, userId, title = 'Spending', global = false }: { groupId?: string; userId?: string; title?: string; global?: boolean }) {
  ...
  const scope = `${groupId || 'all'}:all`;
  const resource = useResource<SpendingInsights>(resourceKeys.insights(userId || 'pending', scope), userId, (signal) => getSpendingInsights(groupId, {}, signal), RESOURCE_FRESHNESS.insights);
  const href = groupId ? `/activity?group=${encodeURIComponent(groupId)}&view=insights&period=all` : '/activity?view=insights&period=all';
  if (!online) return <Surface className="insights-compact"><div className="section-title"><h2>{title}</h2></div><p className="muted" role="status">Spending insights are unavailable offline; counted totals need a complete server aggregate.</p></Surface>;
  if (resource.data === undefined) {
    if (resource.error) return <Surface className="insights-compact"><div className="section-title"><h2>{title}</h2></div><ErrorBox error={resource.error} onRetry={retryFor(resourceKeys.insights(userId || 'pending', scope), userId)} id={`${global ? 'global' : 'group'}-insights-error`} /></Surface>;
    return <Surface className="insights-compact"><div className="section-title"><h2>{title}</h2></div><Loading /></Surface>;
  }
  const summaries = resource.data.summaries;
  const topCategory = resource.data.categories[0];
  return <Surface className="insights-compact"><div className="section-title"><h2>{title}</h2><Link className="inline-action" to={href}>View detailed insights</Link></div>{resource.error || resource.offline ? <p className="cache-status" role="status">Showing the last confirmed spending aggregate; it may be out of date.</p> : null}{summaries.length ? <div className="insight-metrics" aria-label={`${title} by currency`}>{summaries.map((summary) => <InsightCurrencyMetrics key={summary.currency} summary={summary} personal={global} />)}</div> : <p className="muted">No counted expenses yet. Settlements and scheduled expenses are not spending.</p>}{!global && topCategory ? <p className="muted">Top category: <strong>{topCategory.category}</strong> · {topCategory.expenseCount} expense{topCategory.expenseCount === 1 ? '' : 's'}</p> : null}<p className="muted insight-disclosure">{global ? 'Your allocated split spending across active groups.' : 'Group spend uses expense totals; your share uses allocated splits.'} Pending offline expenses are excluded.</p></Surface>;
}
...
function GroupOverview() {
  ...
  return <Layout><Link to="/" className="back">← Groups</Link><div className="page-title"><div><p className="eyebrow">{group.memberCount === 2 ? 'Friend group' : `${group.currency} group`}</p><h1>{displayName}</h1></div><div className="expense-heading__actions"><Link className="button" to={`/groups/${id}/expense/new`}>+ Add expense</Link><Link className="button button--secondary" to={`/groups/${id}/settle`}>Settle up</Link></div></div>{offline ? <ConnectionBanner detail="showing cached group data. New expenses can be captured; history, schedules, and management need a connection." /> : null}{me.error ? <CachedIdentityNotice resource={me} id="group-identity-error" /> : null}{groupResource.error ? <ResourceNotice resource={groupResource} label="group" retry={retryFor(resourceKeys.group(userId, id), me.data?.id)} /> : null}{refreshing ? <p className="cache-status" role="status">Refreshing group data…</p> : null}
    ...
    <section aria-labelledby="people-summary-heading"><div className="section-title"><h2 id="people-summary-heading">People</h2><span className="muted">{members.length} {members.length === 1 ? 'person' : 'people'}</span></div><p className="people-summary-compact">{members.slice(0, 4).map((member) => member.personId === currentPersonId ? 'You' : member.name).join(', ')}{members.length > 4 ? ` +${members.length - 4} more` : ''}</p><Link className="inline-action" to={`/groups/${id}/manage#people`}>Manage people</Link></section>
     <nav className="actions group-tools" aria-label="Group tools"><Link className="button button--secondary" to={`/activity?group=${encodeURIComponent(id)}&view=changes`}>Group History</Link><Link className="button button--secondary" to={`/groups/${id}/manage#settings`}>Group settings</Link></nav>
  </Layout>;
}

function TransactionRow({ groupId, transaction, showNotes = false, currentPersonId }: { groupId: string; transaction: Transaction; showNotes?: boolean; currentPersonId?: string }) {
...
function AuditList({ groupId, entityId, entityType, userId }: { groupId: string; entityId: string; entityType: 'expense' | 'settlement'; userId?: string }) {
  const [open, setOpen] = useState(false);
  return <details className="audit-disclosure" onToggle={(event) => setOpen(event.currentTarget.open)}><summary>View audit history</summary>{open ? <AuditListContent groupId={groupId} entityId={entityId} entityType={entityType} userId={userId} /> : null}</details>;
}
...
function ExpenseDetail() {
  ...
  const expense = details.data?.expense; const groupId = expense?.groupId || id || ''; const group = useResource<GroupResponse>(resourceKeys.group(me.data?.id || 'pending', groupId), me.data?.id, (signal) => getGroup(groupId, signal), RESOURCE_FRESHNESS.group, me.data?.id && groupId ? () => hydrateGroup(me.data!.id, groupId) : undefined); const error = details.error || group.error || me.error;
...
function Settle() {
  ...
  useEffect(() => {
   ...
   useEffect(() => {
     ...
     setTo(members.find((member) => member.personId !== from)?.personId || '');
   }, [from, members, to]);
...
const groupDisplayName = (group: Group) => group.memberCount === 2 && group.counterpartName ? group.counterpartName : group.name;
...
function HistoryTransactions({ groupId, groups }: { groupId?: string; groups: Group[] }) {
  ...
  const [rows, setRows] = useState<Transaction[]>([]); const [cursor, setCursor] = useState<string>(); const [loadingMore, setLoadingMore] = useState(false); const [pageError, setPageError] = useState<unknown>(); const [draftFilters, setDraftFilters] = useState<TransactionFilters>(filters);
  const scopeKey = `${groupId || 'all'}:${filterSignature}`; const scopeKeyRef = useRef(scopeKey); scopeKeyRef.current = scopeKey; const pageScope = useRef(createPageRequestScope()); const cursorRef = useRef<string>();
  ...
  useEffect(() => { const page = transactionsResource.data; if (!page) return; pageScope.current.reset(scopeKey); cursorRef.current = page.nextCursor; setRows(page.transactions); setCursor(page.nextCursor); }, [scopeKey, transactionsResource.data]);
  useEffect(() => () => pageScope.current.dispose(), []);
  useEffect(() => { setDraftFilters(filters); }, [filterSignature]);
  useEffect(() => { if (filters.kind === 'settlement' && searchParams.get('category')) setSearchParams(writeTransactionFilters(searchParams, filters), { replace: true }); }, [filters, searchParams, setSearchParams]);
  ...
  const loadMore = async () => { if (!cursor || loadingMore || offline) return; const request = pageScope.current.begin(scopeKey, cursor); setLoadingMore(true); setPageError(undefined); try { const page = await getGlobalTransactionPage(groupId, { ...filters, limit: 25, cursor: request.cursor }, request.signal); if (!pageScope.current.isCurrent(request) || scopeKeyRef.current !== request.key || cursorRef.current !== request.cursor) return; cursorRef.current = page.nextCursor; setRows((current) => appendUniquePage(current, page.transactions, transactionKey)); setCursor(page.nextCursor); } catch (cause) { if (pageScope.current.isCurrent(request) && !(cause instanceof DOMException && cause.name === 'AbortError')) setPageError(cause); } finally { if (pageScope.current.isCurrent(request)) setLoadingMore(false); } };
  ...
  return <section className="history-panel" aria-labelledby="history-transactions-heading">
    ...
    <TransactionFilterDisclosure filterKey={filterSignature} filterCount={filterCount} offline={offline}><form className="transaction-filters" onSubmit={applyFilters}><Field label="Search"><input type="search" value={draftFilters.q || ''} disabled={offline} onChange={(event) => setDraftFilters((current) => ({ ...current, q: event.target.value || undefined }))} /></Field><Field label="Kind"><select value={draftFilters.kind || ''} disabled={offline} onChange={(event) => setDraftFilters((current) => ({ ...current, kind: (event.target.value || undefined) as TransactionFilters['kind'], category: event.target.value === 'settlement' ? undefined : current.category }))}><option value="">All transaction types</option><option value="expense">Expenses</option><option value="settlement">Settlements</option></select></Field>{groupId ? <Field label="Person"><select value={draftFilters.person || ''} disabled={offline} onChange={(event) => setDraftFilters((current) => ({ ...current, person: event.target.value || undefined }))}><option value="">All people</option>{members.map((member) => <option key={member.personId} value={member.personId}>{member.name}</option>)}</select></Field> : null}<Field label="Category"><select value={draftFilters.category || ''} disabled={offline || draftFilters.kind === 'settlement'} onChange={(event) => setDraftFilters((current) => ({ ...current, category: event.target.value || undefined }))}><option value="">All categories</option>{categoryChoices.map((category) => <option key={category} value={category}>{category}</option>)}</select></Field><Field label="From date"><input type="date" value={draftFilters.from || ''} disabled={offline} onChange={(event) => setDraftFilters((current) => ({ ...current, from: event.target.value || undefined }))} /></Field><Field label="To date"><input type="date" value={draftFilters.to || ''} disabled={offline} onChange={(event) => setDraftFilters((current) => ({ ...current, to: event.target.value || undefined }))} /></Field><Field label="Currency"><select value={draftFilters.currency || ''} disabled={offline} onChange={(event) => setDraftFilters((current) => ({ ...current, currency: event.target.value as Currency || undefined }))}><option value="">All currencies</option>{currencyOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></Field><div className="actions"><Button type="submit" disabled={offline}>Apply</Button>{filterCount ? <Button type="button" variant="secondary" disabled={offline} onClick={clearFilters}>Clear</Button> : null}</div></form></TransactionFilterDisclosure>
     {transactionsResource.data !== undefined ? <ResourceNotice resource={transactionsResource} label="transactions" retry={retryFor(resourceKeys.transactions(userId, groupId || 'all', filterSignature), me.data?.id)} /> : transactionsResource.error ? <ErrorBox error={transactionsResource.error} onRetry={online ? retryFor(resourceKeys.transactions(userId, groupId || 'all', filterSignature), me.data?.id) : undefined} id="history-transactions-error" /> : null}{pending.length ? <section className="pending-transactions" aria-labelledby="pending-transactions-heading"><h3 id="pending-transactions-heading">Waiting to sync · {pending.length}</h3><div className="list">{pending.map((item) => <PendingExpenseRow key={item.clientOperationId} item={item} groupName={!groupId ? groupNames.get(item.groupId) : undefined} />)}</div></section> : null}{transactionsResource.data !== undefined && rows.length ? <div className="list transaction-list">{rows.map((transaction) => <TransactionRow key={transactionKey(transaction)} groupId={transaction.groupId} transaction={transaction} currentPersonId={me.data?.personId} showNotes />)}</div> : transactionsResource.data !== undefined && !pending.length ? <Empty>No transactions match the current filters.</Empty> : null}{cursor ? <Button type="button" variant="secondary" disabled={loadingMore || offline} onClick={() => void loadMore()}>{loadingMore ? 'Loading…' : 'Load more transactions'}</Button> : null}{pageError ? <ErrorBox error={pageError} id="history-transactions-page-error" /> : null}
  </section>;
}
...
function HistoryChanges({ groupId }: { groupId?: string }) {
  ...
  const itemRow = (item: ActivityItem) => { const content = <><span><strong>{item.type.startsWith('settlement') ? `${item.fromName || 'Unknown member'} paid ${item.toName || 'unknown member'}` : item.label || 'Expense'}</strong><small>{item.groupName ? `${item.groupName} · ` : ''}{typeLabel(item.type)} · {item.transactionDate || item.createdAt.slice(0, 10)}</small>{item.label && item.type.startsWith('settlement') ? <small className="activity-description">{item.label}</small> : null}</span>{item.amountMinor != null && item.currency ? <Money amountMinor={item.amountMinor} currency={item.currency} /> : null}</>; const path = transactionActivityPath(item.groupId, item); return path ? <Link className="row" to={path} key={`${item.type}-${item.id}`}>{content}</Link> : <div className="row" key={`${item.type}-${item.id}`}>{content}</div>; };
  return <section className="history-panel" aria-labelledby="history-changes-heading"><h2 id="history-changes-heading" className="sr-only">Changes</h2>{!online || activity.offline ? <p className="offline-banner" role="status">Offline · showing cached history; this is only the first cached page.</p> : null}{me.error ? <CachedIdentityNotice resource={me} id="history-identity-error" /> : null}<ResourceNotice resource={activity} label="history" retry={retryFor(resourceKeys.activity(me.data?.id || 'pending', groupId || 'all'), me.data?.id)} />{activity.data !== undefined && items.length ? <div className="list reading-width">{items.map(itemRow)}</div> : activity.data !== undefined ? <Empty>No history yet.</Empty> : null}{cursor ? <Button type="button" variant="secondary" disabled={loadingMore || !online} onClick={() => void loadMore()}>{loadingMore ? 'Loading…' : 'Load more history'}</Button> : null}{pageError ? <ErrorBox error={pageError} id="history-page-error" /> : null}</section>;
}

function InsightBars({ idPrefix, title, items, getValue, renderLabel }: { idPrefix: string; title: string; items: Array<{ key: string; label: string; value: number; detail?: string }>; getValue: (value: number) => string; renderLabel?: (item: { key: string; label: string; value: number; detail?: string }) => ReactNode }) {
  const headingId = `${idPrefix}-${title.toLowerCase().replace(/[^a-z0-9]+/g, '-')}-heading`;
  const maximum = Math.max(0, ...items.map((item) => item.value));
  return <section className="insight-chart" aria-labelledby={headingId}><h3 id={headingId}>{title}</h3>{items.length ? <ol className="insight-bars" aria-label={`${title} chart`}>{items.map((item) => <li key={item.key}><div className="insight-bar__header"><span>{renderLabel ? renderLabel(item) : item.label}</span><strong>{getValue(item.value)}</strong></div><div className="insight-bar__track" aria-hidden="true"><span style={{ width: `${insightBarWidth(item.value, maximum)}%` }} /></div>{item.detail ? <small>{item.detail}</small> : null}</li>)}</ol> : <p className="muted">No counted expenses in this period.</p>}</section>;
}

function SpendingInsightsPage({ groupId }: { groupId?: string }) {
  const online = useOnlineStatus();
  const [searchParams, setSearchParams] = useSearchParams();
  ...
  const me = useResource(resourceKeys.identity(), '', (signal) => getMe({ signal }), RESOURCE_FRESHNESS.expenses, hydrateIdentity);
  const userId = me.data?.id || 'pending';
  const scope = `${groupId || 'all'}:${query.toString()}`;
  const empty: SpendingInsights = { scope: groupId ? 'group' : 'global', summaries: [], buckets: [], categories: [], ...(groupId ? { participants: [] } : { groups: [] }) };
  const resource = useResource<SpendingInsights>(resourceKeys.insights(userId, scope), me.data?.id, (signal) => !customValid ? Promise.resolve(empty) : getSpendingInsights(groupId, { from: filters.from, to: filters.to, currency: filters.currency }, signal), RESOURCE_FRESHNESS.insights);
  const updateUrl = (next: URLSearchParams) => { next.set('view', 'insights'); setSearchParams(next); };
  const selectPeriod = (period: InsightFilters['period']) => {
    const next = new URLSearchParams(searchParams); next.set('period', period);
    if (period === 'custom') { next.delete('from'); next.delete('to'); setCustomDraft({ from: '', to: '' }); setCustomError(''); }
    else { next.delete('from'); next.delete('to'); }
    updateUrl(next);
  };
  const applyCustom = () => {
    const fromError = customDraft.from !== '' && !validInsightRange(customDraft.from, customDraft.from) ? 'Enter a real start date.' : '';
    const toError = customDraft.to !== '' && !validInsightRange(customDraft.to, customDraft.to) ? 'Enter a real end date.' : '';
    const rangeError = !fromError && !toError && customDraft.from && customDraft.to && customDraft.from > customDraft.to ? 'Start date must not be after end date.' : '';
    if (!customDraft.from || !customDraft.to || fromError || toError || rangeError) { setCustomError(fromError || toError || rangeError || 'Choose both dates.'); return; }
    const next = new URLSearchParams(searchParams); next.set('period', 'custom'); next.set('from', customDraft.from); next.set('to', customDraft.to); setCustomError(''); updateUrl(next);
  };
  const currencies = insightCurrencyChoices(resource.data, filters.currency);
  const selectedCurrency = filters.currency;
  const periodLabel = filters.period === 'month' ? 'This month' : filters.period === 'last-month' ? 'Last month' : filters.period === 'year' ? 'This year' : filters.period === 'custom' ? 'Custom range' : 'All time';
  const data = resource.data;
  const formatMoneyValue = (value: number, currencyValue: Currency) => `${(value / 100).toFixed(2)} ${currencyValue}`;
  const comparisonFor = (currencyValue: Currency) => data?.previous?.summaries.find((item) => item.currency === currencyValue);
  const primaryValue = (summary: SpendingInsights['summaries'][number]) => resource.data!.scope === 'global' ? summary.allocatedSpendMinor : summary.groupSpendMinor;
  const comparisonText = (summary: SpendingInsights['summaries'][number]) => {
    const previous = comparisonFor(summary.currency);
    if (!previous) return 'No previous-period baseline';
    const previousValue = data?.scope === 'global' ? previous.allocatedSpendMinor : previous.groupSpendMinor;
    const change = primaryValue(summary) - previousValue;
    const percent = previousValue === 0 ? (change === 0 ? '0%' : 'new') : `${change >= 0 ? '+' : ''}${((change / previousValue) * 100).toFixed(1)}%`;
    return `${change >= 0 ? '+' : ''}${formatMoneyValue(change, summary.currency)} vs previous (${percent})`;
  };
  return <section className="insights-page" aria-labelledby="insights-heading">
    <div className="section-title"><div><h2 id="insights-heading">Spending insights</h2><p className="muted">{periodLabel}{filters.from && filters.to ? ` · ${filters.from} to ${filters.to}` : ''}</p></div></div>
    <div className="insight-controls" aria-label="Spending insight filters"><Field label="Period"><select value={filters.period} onChange={(event) => selectPeriod(event.target.value as InsightFilters['period'])}><option value="month">This month</option><option value="last-month">Last month</option><option value="year">This year</option><option value="all">All time</option><option value="custom">Custom range</option></select></Field>{filters.period === 'custom' ? <><Field label="From" errorId="insights-custom-from-error" error={customDraft.from !== '' && !validInsightRange(customDraft.from, customDraft.from) ? 'Enter a real start date.' : undefined}><input type="date" value={customDraft.from} aria-invalid={customDraft.from !== '' && !validInsightRange(customDraft.from, customDraft.from)} aria-describedby={customDraft.from !== '' && !validInsightRange(customDraft.from, customDraft.from) ? 'insights-custom-from-error' : undefined} onChange={(event) => { setCustomDraft((current) => ({ ...current, from: event.target.value })); setCustomError(''); }} /></Field><Field label="To" errorId="insights-custom-to-error" error={customDraft.to !== '' && !validInsightRange(customDraft.to, customDraft.to) ? 'Enter a real end date.' : undefined}><input type="date" value={customDraft.to} aria-invalid={customDraft.to !== '' && !validInsightRange(customDraft.to, customDraft.to)} aria-describedby={customDraft.to !== '' && !validInsightRange(customDraft.to, customDraft.to) ? 'insights-custom-to-error' : undefined} onChange={(event) => { setCustomDraft((current) => ({ ...current, to: event.target.value })); setCustomError(''); }} /></Field><Button type="button" aria-describedby={customError ? 'insights-custom-range-error' : undefined} onClick={applyCustom}>Apply range</Button>{customError ? <p id="insights-custom-range-error" className="error" role="alert">{customError}</p> : null}</> : null}{currencies.length > 1 || selectedCurrency ? <Field label="Currency"><select value={selectedCurrency || ''} onChange={(event) => { const next = new URLSearchParams(searchParams); if (event.target.value) next.set('currency', event.target.value); else next.delete('currency'); updateUrl(next); }}><option value="">All currencies</option>{currencies.map((currencyValue) => <option key={currencyValue} value={currencyValue}>{currencyValue}</option>)}</select></Field> : null}</div>
    {!online ? <ConnectionBanner detail="insights are unavailable offline because a complete server aggregate is required." /> : null}
    {filters.period === 'custom' && !customValid ? <p className="muted" role="status">Choose two valid dates and apply the range to load insights.</p> : !online ? <p className="muted" role="status">Detailed insights are unavailable offline. Reconnect for a complete server aggregate.</p> : resource.data === undefined ? resource.error ? <ErrorBox error={resource.error} onRetry={retryFor(resourceKeys.insights(userId, scope), me.data?.id)} id="insights-error" /> : <Loading /> : resource.data.summaries.length === 0 ? <Empty>No counted expenses in this period. Settlements, scheduled templates, deleted expenses, and pending offline expenses are excluded.</Empty> : <><ResourceNotice resource={resource} label="insights" retry={retryFor(resourceKeys.insights(userId, scope), me.data?.id)} /><div className="insight-summary-grid" aria-label="Spending summary metrics">{resource.data.summaries.map((summary) => <div key={summary.currency}><InsightCurrencyMetrics summary={summary} personal={resource.data!.scope === 'global'} /><p className="muted insight-comparison">{comparisonText(summary)}</p></div>)}</div><section className="insight-analysis" aria-labelledby="insight-analysis-heading"><h3 id="insight-analysis-heading">What stands out</h3><ul>{insightSentences(resource.data, selectedCurrency).map((sentence) => <li key={sentence}>{sentence}</li>)}</ul></section>{currencies.filter((currencyValue) => !selectedCurrency || currencyValue === selectedCurrency).map((currencyValue) => { const summary = summaryForCurrency(resource.data!.summaries, currencyValue); const buckets = resource.data!.buckets.filter((item) => item.currency === currencyValue); const categories = categoriesForCurrency(resource.data!.categories, currencyValue); const primaryBucketValue = (item: SpendingInsights['buckets'][number]) => resource.data!.scope === 'global' ? item.allocatedSpendMinor : item.groupSpendMinor; const primaryCategoryValue = (item: SpendingInsights['categories'][number]) => resource.data!.scope === 'global' ? item.allocatedSpendMinor : item.groupSpendMinor; return <section className="insight-currency" key={currencyValue} aria-labelledby={`insight-${currencyValue}-heading`}><h3 id={`insight-${currencyValue}-heading`}>{currencyValue}</h3><div className="insight-currency__metrics"><span>{resource.data!.scope === 'global' ? 'Allocated spending' : 'Group spending'} <Money amountMinor={primaryValue(summary || { currency: currencyValue, groupSpendMinor: 0, allocatedSpendMinor: 0, yourShareMinor: 0, youPaidMinor: 0, expenseCount: 0 })} currency={currencyValue} /></span><span>Your share <Money amountMinor={summary?.yourShareMinor || 0} currency={currencyValue} /></span><span>You paid <Money amountMinor={summary?.youPaidMinor || 0} currency={currencyValue} /></span></div><InsightBars idPrefix={`insight-${currencyValue}`} title="Spending over time" items={buckets.map((item) => ({ key: item.bucket, label: item.bucket, value: primaryBucketValue(item), detail: `${item.expenseCount} counted expense${item.expenseCount === 1 ? '' : 's'} · your share ${formatMoneyValue(item.yourShareMinor, currencyValue)}` }))} getValue={(value) => formatMoneyValue(value, currencyValue)} /><InsightBars idPrefix={`insight-${currencyValue}`} title="Categories" items={categories.map((item) => ({ key: item.category, label: item.category, value: primaryCategoryValue(item), detail: `${item.expenseCount} counted expense${item.expenseCount === 1 ? '' : 's'}` }))} getValue={(value) => formatMoneyValue(value, currencyValue)} renderLabel={(item) => item.label === 'Uncategorized' ? item.label : <Link to={insightTransactionPath(groupId, { ...filters, currency: currencyValue }, item.label)}>{item.label}</Link>} />{resource.data!.scope === 'global' ? <InsightBars idPrefix={`insight-${currencyValue}`} title="Spending by group" items={(resource.data!.groups || []).filter((item) => item.currency === currencyValue).map((item) => ({ key: item.groupId, label: item.groupName, value: item.allocatedSpendMinor, detail: `Your share ${formatMoneyValue(item.yourShareMinor, currencyValue)} · ${item.expenseCount} counted expense${item.expenseCount === 1 ? '' : 's'}` }))} getValue={(value) => formatMoneyValue(value, currencyValue)} renderLabel={(item) => <Link to={insightTransactionPath(item.key, { ...filters, currency: currencyValue })}>{item.label}</Link>} /> : <InsightBars idPrefix={`insight-${currencyValue}`} title="Participant shares" items={(resource.data!.participants || []).filter((item) => item.currency === currencyValue).map((item) => ({ key: item.personId, label: item.name, value: item.shareMinor, detail: `Allocated share in ${currencyValue}` }))} getValue={(value) => formatMoneyValue(value, currencyValue)} />}</section>; })}<p className="muted insight-disclosure">Pending offline expenses are excluded until synced. Scheduled expenses count only after they generate a normal expense; settlements never count as spending.</p></>}
  </section>;
}
   ...
   return <Layout>
    ...
    <section><h2>Pending expenses</h2>{outbox.length ? <div className="list">{outbox.map((item) => <div className="row" key={item.clientOperationId}><span>{item.display.description}<small>{statusLabel(item.status, item.deliveryUncertain)}</small></span><strong>{item.display.currency} {(item.display.amountMinor / 100).toFixed(2)}</strong></div>)}</div> : <p className="muted">New expenses sync automatically when you are online and signed in.</p>}</section>
     ...
     <section><h2>Local data</h2><p className="muted">Clear cached identity, groups, snapshots, and recent preferences without deleting pending or uncertain outbox expenses. Resolve those from the queue controls before removing them.</p><Button variant="secondary" disabled={clearing} onClick={() => void clearCache}>{clearing ? 'Clearing…' : 'Clear cached data'}</Button>{message ? <p className="muted" role="status">{message}</p> : null}{error ? <ErrorBox error={error} /> : null}</section>
      ...
      <section><h2>Account</h2><p className="muted">Logging out revokes the server session first, then clears local account data before Clerk ends the session.</p>{logoutError ? <div className="error" id="logout-error" role="alert" aria-live="assertive"><strong>Logout was not completed.</strong> <span>{errorText(logoutError)}</span></div> : null}<div className="actions"><Button variant="danger" disabled={!outboxReady || clearing || exporting || deletingAccount || !online} onClick={() => void logout()}>{outboxReady ? clearing ? 'Signing out securely…' : 'Log out' : 'Checking pending expenses…'}</Button><Button variant="secondary" disabled={!outboxReady || clearing || exporting || deletingAccount || !online} onClick={() => void logout(true)}>Log out all devices</Button></div></section>
...
function PrivateRoutes() {
  ...
  return <Routes key={identityEpoch}><Route path="/" element={<Home />} /><Route path="/settings" element={<Settings />} /><Route path="/activity" element={<History />} /><Route path="/expense/new" element={<ExpenseForm />} /><Route path="/groups/:id" element={<GroupOverview />} /><Route path="/groups/:id/transactions" element={<LegacyTransactionRedirect />} /><Route path="/groups/:id/manage" element={<GroupManagementPage />} /><Route path="/groups/:id/expense/new" element={<ExpenseForm />} /><Route path="/groups/:id/expense/:expenseId" element={<ExpenseForm />} /><Route path="/groups/:id/scheduled-expense/new" element={<LegacyScheduledExpenseRedirect />} /><Route path="/groups/:id/scheduled-expense/:scheduledExpenseId" element={<ExpenseForm />} /><Route path="/groups/:id/expenses/:expenseId" element={<ExpenseDetail />} /><Route path="/expenses/:expenseId" element={<ExpenseDetail />} /><Route path="/groups/:id/settle" element={<Settle />} /><Route path="/groups/:id/settlements/:settlementId" element={<SettlementDetail />} /><Route path="/groups/:id/activity" element={<LegacyActivityRedirect />} /><Route path="*" element={<Navigate to="/" replace />} /></Routes>;
}
