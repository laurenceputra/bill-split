...
function Home() {
  ...
  useEffect(() => { getGroups().then((result) => { setGroups(result.groups); setOffline(Boolean(result.offline)); }).catch(setError); }, []);
  useEffect(() => {
...
function GroupPage() {
  ...
  const load = () => Promise.all([getGroup(id), getExpenses(id), getBalances(id), getSettlements(id), getMe()]).then(([groupResult, expenseResult, balanceResult, settlementResult, me]) => {
    setGroup(groupResult.group); setMembers(groupResult.members); setExpenses(expenseResult.expenses); setBalances(balanceResult.balances); setSettlements(settlementResult.settlements); setCurrentPersonId(me.personId); setCurrentUserId(me.id); setOffline(Boolean(groupResult.offline || expenseResult.offline || balanceResult.offline || settlementResult.offline || me.offline)); setPending(getOutboxSnapshot().filter((item) => item.userId === me.id && item.groupId === id));
  }).catch(setError);
  useEffect(() => { void load(); }, [id]);
  useEffect(() => { const unsubscribe = subscribeOutbox(() => { setPending(getOutboxSnapshot().filter((item) => item.userId === currentUserId && item.groupId === id)); void load(); }); return () => { unsubscribe(); }; }, [id, currentUserId]);
  ...
  return <Layout>
    ...
    {offlineView ? <p className="offline-banner" role="status">Offline · stale data is available. Only new expenses can be captured; settle, activity, exports, and member changes require a connection.</p> : null}
...
function ExpenseForm() {
  ...
  useEffect(() => {
    ...
    Promise.all([getGroup(id), getMe(), expenseId ? getExpenseDetails(expenseId) : Promise.resolve(undefined)]).then(([groupResult, me, details]) => {
      if (!active) return;
       setGroup(groupResult.group); setMembers(groupResult.members); setCurrentPersonId(me.personId); setCurrentUserId(me.id); setOfflineData(Boolean(groupResult.offline || me.offline));
    ...
    }).catch((cause) => { if (cause instanceof ApiError && cause.networkFailure) setOfflineData(true); setError(cause); });
    return () => { active = false; };
  }, [id, expenseId]);
  ...
  return <Layout>
    <div className="page-title expense-heading"><div><Link to={`/groups/${id}`} className="back">← <span className="back__label">{group.name}</span></Link><p className="eyebrow">{expenseId ? 'Edit expense' : 'New expense'}</p><h1>{expenseId ? 'Edit expense' : 'Add expense'}</h1></div><div className="expense-heading__actions"><Link className="button button--secondary" to={`/groups/${id}`}>Cancel</Link></div></div>
     {editUnavailable ? <p className="offline-banner" role="status">Editing expenses is online-only. Reconnect before saving changes.</p> : null}<form className="expense-form" onSubmit={submit}>
...
function Settle() {
  ...
  const [error, setError] = useState<unknown>();
  const [offlineData, setOfflineData] = useState(false);
  useEffect(() => { Promise.all([getMe(), getGroup(id), getBalances(id)]).then(([me, groupResult, balanceResult]) => { setOfflineData(Boolean(me.offline || groupResult.offline || balanceResult.offline)); setCurrentPersonId(me.personId); setGroup(groupResult.group); setMembers(groupResult.members); const suggestion = settlementSuggestion(balanceResult.balances, me.personId, groupResult.group.currency); setCurrency(suggestion?.currency || groupResult.group.currency); setFrom(suggestion?.fromPersonId || groupResult.members[0]?.personId || ''); setTo(suggestion?.toPersonId || groupResult.members.find((member) => member.personId !== (suggestion?.fromPersonId || groupResult.members[0]?.personId))?.personId || ''); setAmount(suggestion ? moneyInput(suggestion.amountMinor) : ''); }).catch((cause) => { if (cause instanceof ApiError && cause.networkFailure) setOfflineData(true); setError(cause); }); }, [id]);
  if (!group) return <Layout>{error ? <ErrorBox error={error} /> : <Loading />}</Layout>;
   if (!online || offlineData) return <Layout><Link to={`/groups/${id}`} className="back">← <span className="back__label">{group.name}</span></Link><p className="offline-banner" role="status">Settlements are online-only. Reconnect to settle up.</p></Layout>;
...
function Activity() {
  ...
  const [error, setError] = useState<unknown>();
  const [offlineData, setOfflineData] = useState(false);
  useEffect(() => { if (online) api<{ activity: Array<{ type: string; id: string; label: string | null; createdAt: string }> }>(`/groups/${id}/activity`).then((result) => setItems(result.activity)).catch((cause) => { if (cause instanceof ApiError && cause.networkFailure) setOfflineData(true); setError(cause); }); }, [id, online]);
