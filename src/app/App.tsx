...
function GroupSettings({ group, groupId, userId, online, role, onDeleted, onLeft }: { group: Group; groupId: string; userId: string; online: boolean; role: 'owner' | 'member'; onDeleted: () => void; onLeft: () => void }) {
  ...
  const leave = async () => {
    if (!online || busy || !confirm(`Leave ${displayName}? You will lose access to this group. Historical transactions are retained for the group.`)) return;
  ...
  const convert = async (event: FormEvent) => {
    event.preventDefault();
    if (!online || busy || !name.trim()) { if (!name.trim()) setError(new Error('Enter a name for the named group.')); return; }
    setBusy('convert'); setError(undefined);
    try { await convertPeerToNamedGroup(groupId, name.trim()); await invalidateForMutation.groupChanged(groupId, userId, captureSessionGeneration()); }
    catch (cause) { setError(cause); }
    finally { setBusy(undefined); }
  };
