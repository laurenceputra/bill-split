...
function GroupManagementPage() {
  ...
  const groupResource = useResource<{ group: Group; members: GroupMember[]; splitDefault: GroupSplitDefault | null }>(resourceKeys.group(userId, id), me.data?.id, (signal) => getGroup(id, signal), RESOURCE_FRESHNESS.group, me.data?.id ? () => hydrateGroup(me.data!.id, id) : undefined);
...
function ExpenseForm() {
  ...
  const groupResource = useResource<{ group: Group; members: GroupMember[]; splitDefault: GroupSplitDefault | null }>(resourceKeys.group(formUserId, id || 'none'), meResource.data?.id, (signal) => id ? getGroup(id, signal) : Promise.resolve({ group: undefined as unknown as Group, members: [], splitDefault: null }), RESOURCE_FRESHNESS.group, meResource.data?.id && id ? () => hydrateGroup(meResource.data!.id, id) : undefined);
     ...
     setCurrency(expense?.currency ?? schedule?.currency ?? groupResult.group.currency);
      if (expense || schedule) {
       ...
       if ('startDate' in record) { setEndDate(record.endDate || ''); setFrequency(record.frequency); setInterval(String(record.interval)); setWeekdays(record.weekdays); setTimezone(record.timezone); setCustomTimezone(''); setUsingCustomTimezone(false); }
          ...
          const payer = currentPayerSelection(me.personId, groupResult.members), resolvedDefault = resolveGroupSplitDefault(groupResult.splitDefault, groupResult.members);
...
function HistoryTransactions({ groupId, groups }: { groupId?: string; groups: Group[] }) {
  ...
  const me = useResource(resourceKeys.identity(), '', (signal) => getMe({ signal }), RESOURCE_FRESHNESS.expenses, hydrateIdentity);
  const userId = me.data?.id || 'pending';
  const groupResource = useResource<{ group?: Group; members: GroupMember[] }>(resourceKeys.group(userId, groupId || 'all'), me.data?.id, (signal) => groupId ? getGroup(groupId, signal) : Promise.resolve({ group: undefined, members: [] }), RESOURCE_FRESHNESS.group, groupId && me.data?.id ? () => hydrateGroup(me.data!.id, groupId) : undefined);
