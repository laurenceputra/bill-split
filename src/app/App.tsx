...
import { AuthLoadingShell, Avatar, AvatarStack, Button, EmptyState, Field, InstallAction, Layout, Modal, Money, PublicShell, Skeleton, SplitTransactionControl, Status, Surface, connectionStatusLabel, useAuthLifecycle, useConnectionState, useOnlineStatus } from './ui';
...
function CompactInsights({ groupId, userId, title = 'Spending', global = false }: { groupId?: string; userId?: string; title?: string; global?: boolean }) {
  ...
  if (resource.data === undefined) {
    if (!online) return <section className="insights-compact"><div className="section-title"><h2>{title}</h2></div><p className="muted" role="status">Refresh is unavailable offline; cached spending data may be shown when available.</p></section>;
    if (resource.error) return <section className="insights-compact"><div className="section-title"><h2>{title}</h2></div><ErrorBox error={resource.error} onRetry={retryFor(resourceKeys.insights(userId || 'pending', scope), userId)} id={`${global ? 'global' : 'group'}-insights-error`} /></section>;
    return <section className="insights-compact"><div className="section-title"><h2>{title}</h2></div><Loading /></section>;
  }
  const summaries = resource.data.summaries;
  return <section className="insights-compact"><div className="section-title"><h2>{title}</h2><Link className="inline-action" to={href}>View detailed insights</Link></div>{!online ? <p className="cache-status" role="status">Refresh is unavailable offline; showing cached spending data.</p> : resource.error || resource.offline ? <p className="cache-status" role="status">Showing the last confirmed spending aggregate; it may be out of date.</p> : null}{summaries.length ? <div className="insight-metrics" aria-label={`${title} by currency`}>{summaries.map((summary) => <InsightCurrencyMetrics key={summary.currency} summary={summary} personal={global} />)}</div> : <p className="muted">No counted expenses yet. Settlements and scheduled expenses are not spending.</p>}<p className="muted insight-disclosure">{global ? 'Your allocated split spending across active groups.' : 'Group spend uses expense totals; your share uses allocated splits.'} Pending offline expenses are excluded.</p></section>;
}
...
function GroupSettings({ group, groupId, userId, online, role, activeMemberCount, pendingGenericInvitation, conversionBusy = false, onConversionBusyChange, onDeleted, onLeft }: { group: Group; groupId: string; userId: string; online: boolean; role: 'owner' | 'member'; activeMemberCount: number; pendingGenericInvitation?: boolean; conversionBusy?: boolean; onConversionBusyChange?: (busy: boolean) => void; onDeleted: () => void; onLeft: () => void }) {
  ...
  const relationshipSettings = managementKind === 'named' ? <section className="group-relationship-settings" aria-labelledby="relationship-type-heading"><div className="section-title"><h2 id="relationship-type-heading">Relationship type</h2><span className="muted">Owner-only</span></div><p className="muted">Peer relationships use the other participant’s name as the ledger name and cannot add more people until converted back to a named group.</p>{activeMemberCount === 2 && pendingGenericInvitation === false ? <Button type="button" disabled={!online || Boolean(busy)} onClick={() => void convertToPeer()}>{busy === 'convert' ? 'Converting…' : 'Convert to peer relationship'}</Button> : <div className="muted" role="status"><p>Peer conversion is currently unavailable:</p><ul>{activeMemberCount !== 2 ? <li>Exactly two active ledger participants are required.</li> : null}{pendingGenericInvitation === true ? <li>Revoke pending generic group invitations first.</li> : null}{pendingGenericInvitation === undefined ? <li>Reconnect to confirm the group invitation status.</li> : null}</ul></div>}</section> : null;
...
function Home() {
  ...
  const addFriendRequested = searchParams.get('friend') === '1';
   ...
   if (coldHomeLoading) return <Layout><HomeLoadingPlaceholder /></Layout>;
    return <Layout>
      <div className="page-title"><div><p className="eyebrow">Private expenses</p><h1>Friends &amp; groups</h1></div><div className="home-actions"><Link className="button" to="/friends/new">+ Add friend</Link><Link className="button button--secondary" to="/groups/new">New group</Link></div></div>
       ...
       {groupsResource.data === undefined && !me.error ? <ResourceNotice resource={groupsResource} label="groups" retry={retryFor(resourceKeys.groups(me.data?.id || 'pending'), me.data?.id)} /> : groupsResource.data !== undefined ? <><ResourceNotice resource={groupsResource} label="groups" retry={retryFor(resourceKeys.groups(me.data?.id || 'pending'), me.data?.id)} />{groups.length ? <div className="cards group-cards">{groups.map((group) => { const people = [{ name: 'You' }, ...(group.counterpartName ? [{ name: group.counterpartName }] : [])]; return <Link className="card group-card" to={`/groups/${group.id}`} key={group.id}><div className="group-card__identity"><div className="group-card__heading"><AvatarStack people={people} /><strong className="card__name">{groupDisplayName(group)}</strong></div><span className="group-card__meta">{group.memberCount ? `${group.memberCount} ${group.memberCount === 1 ? 'person' : 'people'}` : group.kind === 'peer' ? 'Friend ledger' : 'Shared ledger'} · {group.currency}</span></div><div className="card__balances">{groupBalanceDisplays(group.balanceSummaries, group.currency).map((display, index) => display.kind === 'balance' ? <span className="card__balance" key={`${display.currency}-${index}`}><span className="card__balance-label">{display.label}</span><span className="card__balance-money"><Money amountMinor={display.amountMinor} currency={display.currency} tone={display.label === 'You are owed' ? 'positive' : 'debt'} /><small>{display.currency}</small></span></span> : <span className={`card__balance card__balance--${display.kind}`} key={`${display.label}-${index}`}><span className="card__balance-label">{display.label}</span><small>{display.currency}</small></span>)}</div></Link>; })}</div> : <EmptyState title="No groups yet"><p className="empty-state__copy">Start a friend ledger for a quick one-to-one split, or create a shared group for a trip or household.</p><div className="empty-state__actions"><Link className="button" to="/friends/new">Add a friend</Link><Link className="button button--secondary" to="/groups/new">Create a group</Link></div></EmptyState>}</> : null}
...
function CreationPageLayout({ eyebrow, title, children }: { eyebrow: string; title: string; children: ReactNode }) {
  return <Layout><Link className="back" to="/">← Friends &amp; groups</Link><div className="page-title"><div><p className="eyebrow">{eyebrow}</p><h1>{title}</h1></div></div>{children}</Layout>;
}
...
function FriendCreationPage() {
  ...
  return <CreationPageLayout eyebrow="New connection" title="Add friend">{!online ? <ConnectionBanner detail="Friend creation requires a connection." /> : null}<Surface><p className="muted">The signed-in account is added as the owner. An email only creates a targeted invitation; it does not grant access until accepted.</p><form onSubmit={submit} aria-describedby={error ? 'create-friend-error' : undefined}><Field label="Friend name"><input id="friend-name" required value={name} onChange={(event) => { setError(undefined); setName(event.target.value); }} /></Field><Field label="Email (optional)"><input id="friend-email" className="email" type="email" value={email} onChange={(event) => { setError(undefined); setEmail(event.target.value); }} /></Field><Field label="Currency"><CurrencySelect value={currency} onChange={setCurrency} /></Field>{error ? <ErrorBox error={error} id="create-friend-error" /> : null}<div className="actions"><Button type="submit" disabled={!online || busy}>{busy ? 'Adding…' : 'Add friend'}</Button><Link className="button button--secondary" to="/">Cancel</Link></div></form></Surface></CreationPageLayout>;
}
...
function OwnerGroupManagement({ group, groupId, userId, members, currentPersonId, online, offline, groupResource, splitDefault, onSplitDefaultChanged, onDeleted, onLeft }: GroupManagementContentProps) {
  ...
  const pendingGenericInvitation = invitationsResource.data !== undefined && !invitationsResource.error && !invitationsResource.revalidating && !invitationsResource.stale && !invitationsResource.offline
    ...
    : undefined;
  return <GroupManagementLayout group={group} groupId={groupId} userId={userId} online={online} offline={offline} groupResource={groupResource} activeMemberCount={members.length} pendingGenericInvitation={pendingGenericInvitation} conversionBusy={conversionBusy} onConversionBusyChange={setConversionBusy} onDeleted={onDeleted} onLeft={onLeft}><PeopleSection groupId={groupId} userId={userId} members={members} currentPersonId={currentPersonId} online={effectiveOnline && !conversionBusy} owner invitationsResource={invitationsResource} targetedMutationState={targetedInvitationMutationState(invitationsResource)} />{groupManagementKind(group) === 'named' ? <><AddFriendForm groupId={groupId} userId={userId} online={effectiveOnline && !conversionBusy} /><GenericInvitationControls groupId={groupId} userId={userId} online={effectiveOnline && !conversionBusy} invitationsResource={invitationsResource} /></> : groupManagementKind(group) === 'unknown' ? <p className="muted" role="status">Additional people and invitations are unavailable until this group’s type is confirmed online.</p> : null}<SplitDefaultSettings groupId={groupId} userId={userId} members={members} value={splitDefault} online={effectiveOnline && !conversionBusy} owner onChanged={onSplitDefaultChanged} /></GroupManagementLayout>;
}
...
function GroupOverview() {
  ...
  return <Layout>
    ...
    <header className="page-title group-overview-header"><div><h1>{displayName}</h1><p className="group-overview-meta">{memberCount} {memberCount === 1 ? 'member' : 'members'} · {group.currency} · {groupContext}</p></div><div className="expense-heading__actions"><SplitTransactionControl groupId={id} online={online && !offline} primaryAriaLabel="+ Add expense" /><Link className="button button--secondary" to={`/groups/${id}/settle`}>Settle up</Link></div></header>
    ...
    <div className="group-overview-columns">
      <div className="group-overview-main">
        <section className="group-overview-card group-overview-card--balances compact-balances" aria-labelledby="balances-heading"><div className="section-title"><h2 id="balances-heading">Your balances</h2>{knownBalanceCurrencyCount !== undefined ? <span className="muted">{knownBalanceCurrencyCount} {knownBalanceCurrencyCount === 1 ? 'currency' : 'currencies'}</span> : null}</div>{balancesResource.data !== undefined ? <ResourceNotice resource={balancesResource} label="balances" retry={retryFor(resourceKeys.balances(userId, id), me.data?.id)} /> : balancesResource.error ? <ErrorBox error={balancesResource.error} onRetry={online ? retryFor(resourceKeys.balances(userId, id), me.data?.id) : undefined} id="balances-error" /> : null}<ul className="balance-cards">{balancesResource.data === undefined ? <li><p className="cache-status">Balances are unavailable until this group’s balance data is loaded.</p></li> : balanceDisplays.map((display, index) => <li className={`balance-card balance-card--${display.kind} ${display.label === 'You are owed' ? 'balance-card--positive' : display.label === 'You owe' ? 'balance-card--debt' : ''}`} key={`${display.currency}-${index}`}><span className="balance-card__currency">{display.currency}</span><strong>{display.label}</strong>{display.kind === 'balance' ? <Money amountMinor={display.amountMinor} currency={display.currency} tone={display.label === 'You are owed' ? 'positive' : 'debt'} size="large" /> : null}</li>)}</ul><details className="balance-breakdown"><summary>View full group breakdown</summary><p className="muted">Net position = paid minus share, adjusted by settlements. Positive means owed; negative means owes.</p>{balancesResource.data === undefined ? <p className="cache-status">Balance details are unavailable.</p> : Object.keys(balances).length ? Object.entries(balances).map(([currencyKey, balance]) => <div key={currencyKey}><h3>{currencyKey}</h3><ul className="list">{balance.raw.map((item) => <li className="row" key={`${currencyKey}-raw-${item.personId}`}><span>{item.personId === currentPersonId ? 'You' : item.name}</span><span>{balanceStatus(item.netMinor) === 'owed' ? 'is owed' : balanceStatus(item.netMinor) === 'owes' ? 'owes' : 'settled up'}</span>{item.netMinor === 0 ? null : <Money amountMinor={Math.abs(item.netMinor)} currency={currencyKey} tone={item.netMinor > 0 ? 'positive' : 'debt'} />}</li>)}</ul><h4>Suggested settlements</h4>{balance.suggestions.length ? <ul className="list">{balance.suggestions.map((suggestion) => <li className="row" key={`${currencyKey}-suggestion-${suggestion.fromPersonId}-${suggestion.toPersonId}`}><span>{suggestion.fromPersonId === currentPersonId ? 'You' : nameOf(members, suggestion.fromPersonId, suggestion.fromName)} → {suggestion.toPersonId === currentPersonId ? 'You' : nameOf(members, suggestion.toPersonId, suggestion.toName)}</span><Money amountMinor={suggestion.amountMinor} currency={currencyKey} /></li>)}</ul> : <p className="muted">No payment is suggested in this currency.</p>}</div>) : <Empty>No balance details yet.</Empty>}</details></section>
        <section className="group-overview-card group-overview-card--transactions" aria-labelledby="recent-transactions-heading"><div className="section-title"><h2 id="recent-transactions-heading">Recent transactions</h2><Link className="inline-action" to={`/activity?group=${encodeURIComponent(id)}&view=transactions`}>See all</Link></div>{transactionsResource.data !== undefined ? <ResourceNotice resource={transactionsResource} label="transactions" retry={retryFor(resourceKeys.transactions(userId, id, 'overview'), me.data?.id)} /> : transactionsResource.error ? <ErrorBox error={transactionsResource.error} onRetry={online ? retryFor(resourceKeys.transactions(userId, id, 'overview'), me.data?.id) : undefined} id="overview-transactions-error" /> : transactionLoading ? <p className="cache-status" role="status">Loading recent transactions…</p> : null}{pending.length ? <section className="pending-transactions" aria-labelledby="pending-transactions-heading"><h3 id="pending-transactions-heading">Waiting to sync · {pending.length}</h3><div className="list">{pending.map((item) => <PendingExpenseRow key={item.clientOperationId} item={item} />)}</div></section> : null}{transactionsResource.data === undefined ? transactionLoading ? null : <p className="cache-status">Recent transactions are unavailable until this group’s transaction data is loaded.</p> : !transactions.length && !pending.length ? <Empty>No transactions yet.</Empty> : transactions.length ? <ul className="list transaction-list">{transactions.map((transaction) => <li key={transactionKey(transaction)}><TransactionRow groupId={id} transaction={transaction} currentPersonId={currentPersonId} overview /></li>)}</ul> : null}</section>
      </div>
...
function TransactionRow({ groupId, transaction, showNotes = false, currentPersonId, overview = false }: { groupId: string; transaction: Transaction; showNotes?: boolean; currentPersonId?: string; overview?: boolean }) {
  ...
  return path ? <Link className={`row transaction-row${overview ? ' transaction-row--overview' : ''}`} to={path}>{content}</Link> : <div className={`row transaction-row${overview ? ' transaction-row--overview' : ''}`}>{content}</div>;
}
...
function CompactScheduleList(props: ScheduleListProps) {
  ...
  return <section className="scheduled-summary group-overview-card group-overview-card--schedules" aria-labelledby="scheduled-summary-heading"><div className="section-title"><h2 id="scheduled-summary-heading">Scheduled expenses</h2>{props.resource.data !== undefined ? <span className="muted">{scheduleOverviewMetadata(active.length, next, Boolean(props.resource.data.nextCursor))}</span> : null}</div>{scheduleContent}{props.resource.data !== undefined ? <details><summary>View all schedules</summary><div className="schedule-list-content"><ScheduleList {...props} /></div></details> : null}</section>;
}
...
function TransactionChooser() {
  ...
  return <Layout><Link className="back" to={id ? `/groups/${id}` : '/'}>← {id ? 'Group' : 'Groups'}</Link><div className="page-title"><div><p className="eyebrow">Quick ledger action</p><h1>Add transaction</h1></div></div><Surface><p className="muted">Choose what happened. Refunds and reimbursements can start from an expense, or be recorded for more than one expense.</p>{id ? group ? <><h2>{groupDisplayName(group)}</h2>{options(id)}</> : <p role="alert">This group is unavailable. Reconnect or return to Groups; no other group actions are shown.</p> : (groups.data?.groups || []).length ? <div className="chooser-groups">{sortOptionsByLabel(groups.data?.groups || [], groupDisplayName, (candidate) => candidate.id).map((candidate) => <section key={candidate.id}><h2>{groupDisplayName(candidate)}</h2>{options(candidate.id)}</section>)}</div> : <Empty>No groups yet. <Link to="/groups/new">Create a group</Link> or <Link to="/friends/new">add a friend</Link>.</Empty>}</Surface></Layout>;
}
...
function CreditDetail() {
  ...
  return <Layout><Link to={`/groups/${credit.groupId}`} className="back">← Group</Link><div className="page-title"><div><p className="eyebrow">{credit.date} · Refund/reimbursement</p><h1>{credit.deliveryMode === 'direct_provider_offset' ? 'Original payment or bill was adjusted' : 'A group member received the money'}</h1></div><Money amountMinor={credit.amountMinor} currency={credit.currency} size="large" /></div>{!online ? <ConnectionBanner detail="Refund changes require a connection." /> : null}{details.error || group.error ? <p className="cache-status" role="status">Showing cached refund details; names or linked expenses may be out of date.</p> : null}{credit.deletedAt ? <Surface><strong>Deleted refund/reimbursement</strong><p className="muted">This record can be restored within 30 days.</p><Button disabled={!online || busy} onClick={() => void restore()}>{busy ? 'Restoring…' : 'Restore refund/reimbursement'}</Button></Surface> : <><Surface><h2>Money flow</h2>{credit.note ? <p>{credit.note}</p> : <p className="muted">No note was added.</p>}<p>Refund/reimbursement total: <Money amountMinor={credit.amountMinor} currency={credit.currency} /></p><p className="muted">This reduces the affected costs by the same amount in the group balance.</p></Surface><Surface><h2>Linked expenses</h2>{credit.applications.length ? <div className="list">{credit.applications.map((application) => <Link className="row" key={application.expenseId} to={expenseDetailPath(credit.groupId, application.expenseId) || `/groups/${credit.groupId}`}><span><strong>{application.expenseDescription || 'Expense'}</strong>{application.expenseDate ? <small>{application.expenseDate}</small> : null}</span><Money amountMinor={application.amountMinor} currency={credit.currency} /></Link>)}</div> : <p className="muted">Standalone refund/reimbursement.</p>}</Surface>{credit.deliveryMode === 'member_reimbursement' ? <Surface><h2>Who received the money?</h2><div className="list">{recipients.map((allocation) => <div className="row" key={`recipient-${allocation.personId}`}><span>{label(allocation.personId)}</span><Money amountMinor={allocation.amountMinor} currency={credit.currency} /></div>)}</div><h2>Whose costs did this reduce?</h2><div className="list">{affected.map((allocation) => <div className="row" key={`affected-${allocation.personId}`}><span>{label(allocation.personId)}</span><Money amountMinor={allocation.amountMinor} currency={credit.currency} /></div>)}</div></Surface> : <Surface><h2>Original payment or bill was adjusted</h2><p className="muted">The original payer and affected shares were derived from the linked expenses and saved with this refund.</p></Surface>}<div className="actions"><Link className="button button--secondary" to={`/groups/${credit.groupId}/credit/${credit.id}/edit`}>Edit refund/reimbursement</Link><Button type="button" variant="danger" disabled={!online || busy} onClick={() => void remove()}>{busy ? 'Deleting…' : 'Delete refund/reimbursement'}</Button></div></>}{error ? <ErrorBox error={error} id="refund-detail-action-error" /> : null}<details className="reading-width"><summary>Audit history</summary>{details.data?.history.length ? <div className="list">{details.data.history.map((item) => <div className="row" key={item.id}><span>Version {item.revision}</span><small>{new Date(item.createdAt).toLocaleString()}</small></div>)}</div> : <p className="muted">No audit history.</p>}</details></Layout>;
}
...
function ExpenseDetail() {
  ...
  return <Layout><Link to={`/groups/${expense.groupId}`} className="back">← Group</Link><div className="page-title"><div><p className="eyebrow">{expense.deletedAt ? 'Deleted transaction' : expense.date}</p><h1>{expense.description}</h1></div><Money amountMinor={expense.amountMinor} currency={expense.currency} size="large" /></div><ResourceNotice resource={details} label="expense details" retry={retryFor(resourceKeys.expenseDetail(me.data?.id || 'pending', expenseId), me.data?.id)} />{expense.deletedAt ? <Surface><strong>Deleted expense</strong><p className="muted">This expense is retained for 30 days and cannot receive a refund.</p><Button disabled={!online || busy} onClick={() => void restore()}>{busy ? 'Restoring…' : 'Restore expense'}</Button></Surface> : null}<Surface><h2>Cost after refunds/reimbursements</h2><div className="list"><div className="row"><span>Gross expense</span><Money amountMinor={summary.grossMinor} currency={expense.currency} /></div><div className="row"><span>Refunds/reimbursements</span><Money amountMinor={summary.refundedMinor} currency={expense.currency} tone="positive" /></div><div className="row"><span>Net group cost</span><Money amountMinor={summary.netGroupCostMinor} currency={expense.currency} /></div><div className="row"><span>Remaining refundable</span><Money amountMinor={summary.remainingRefundableMinor} currency={expense.currency} /></div></div>{!expense.deletedAt && !summary.fullyRefunded ? <Link className="button" to={refundPath}>Add refund</Link> : <p className="muted" role="status">{refundActionLabel(expense)}</p>}</Surface>{expense.linkedCredits?.length ? <Surface><h2>Refunds/reimbursements</h2><div className="list">{expense.linkedCredits.map((credit) => <Link className="row" key={credit.creditId} to={creditDetailPath(expense.groupId, credit.creditId) || `/groups/${expense.groupId}`}><span>{credit.subtype === 'refund' ? 'Refund' : 'Reimbursement'}<small>{credit.date} · {credit.deliveryMode === 'direct_provider_offset' ? 'Original payment or bill adjusted' : 'A group member received the money'}</small></span><Money amountMinor={credit.amountMinor} currency={expense.currency} /></Link>)}</div></Surface> : null}<section className="reading-width"><h2>Payers</h2><div className="list">{expense.payers.map((payer) => <div className="row" key={payer.personId}><span>{personLabel(payer.personId)}</span><Money amountMinor={payer.amountMinor} currency={expense.currency} /></div>)}</div><h2>Split</h2><div className="list">{expense.splits.map((split) => <div className="row" key={split.personId}><span>{personLabel(split.personId)}</span><Money amountMinor={split.amountMinor} currency={expense.currency} /></div>)}</div></section>{mutationError ? <ErrorBox error={mutationError} id="expense-mutation-error" /> : null}{!expense.deletedAt && online ? <div className="actions"><Link className="button" to={`/groups/${expense.groupId}/expense/${expense.id}`}>Edit</Link><Button variant="danger" disabled={busy} onClick={() => void remove()}>Delete</Button></div> : null}<AuditList groupId={expense.groupId} entityId={expense.id} entityType="expense" userId={me.data?.id} /></Layout>;
}
  ...
  const clearFilters = () => setSearchParams(writeTransactionFilters(searchParams, {}));
   return <section className="history-panel" aria-labelledby="history-transactions-heading">
    ...
    <TransactionFilterDisclosure filterKey={filterSignature} filterCount={filterCount} offline={offline}><form className="transaction-filters" onSubmit={applyFilters}><Field label="Search"><input type="search" value={draftFilters.q || ''} disabled={offline} onChange={(event) => setDraftFilters((current) => ({ ...current, q: event.target.value || undefined }))} /></Field><Field label="Kind"><select value={draftFilters.kind || ''} disabled={offline} onChange={(event) => setDraftFilters((current) => ({ ...current, kind: (event.target.value || undefined) as TransactionFilters['kind'], category: event.target.value === 'settlement' || event.target.value === 'credit' ? undefined : current.category }))}><option value="">All transaction types</option><option value="expense">Expenses</option><option value="settlement">Payments</option><option value="credit">Refunds/reimbursements</option></select></Field>{groupId ? <Field label="Person"><select value={draftFilters.person || ''} disabled={offline} onChange={(event) => setDraftFilters((current) => ({ ...current, person: event.target.value || undefined }))}><option value="">All people</option>{members.map((member) => <option key={member.personId} value={member.personId}>{member.name}</option>)}</select></Field> : null}<Field label="Category"><select value={draftFilters.category || ''} disabled={offline || draftFilters.kind === 'settlement' || draftFilters.kind === 'credit'} onChange={(event) => setDraftFilters((current) => ({ ...current, category: event.target.value || undefined }))}><option value="">All categories</option>{categoryChoices.map((category) => <option key={category} value={category}>{category}</option>)}</select></Field><Field label="From date"><input type="date" value={draftFilters.from || ''} disabled={offline} onChange={(event) => setDraftFilters((current) => ({ ...current, from: event.target.value || undefined }))} /></Field><Field label="To date"><input type="date" value={draftFilters.to || ''} disabled={offline} onChange={(event) => setDraftFilters((current) => ({ ...current, to: event.target.value || undefined }))} /></Field><Field label="Currency"><select value={draftFilters.currency || ''} disabled={offline} onChange={(event) => setDraftFilters((current) => ({ ...current, currency: event.target.value as Currency || undefined }))}><option value="">All currencies</option>{currencyOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></Field><div className="actions"><Button type="submit" disabled={offline}>Apply</Button>{filterCount ? <Button type="button" variant="secondary" disabled={offline} onClick={clearFilters}>Clear</Button> : null}</div></form></TransactionFilterDisclosure>
      {filteredOfflineUnavailable ? <p className="cache-status" role="status">Filtered transaction results aren’t cached on this device. Reconnect to search history.</p> : <ResourceNotice resource={transactionsResource} label="transactions" retry={retryFor(resourceKeys.transactions(userId, groupId || 'all', filterSignature), me.data?.id)} />}{pending.length ? <section className="pending-transactions" aria-labelledby="pending-transactions-heading"><h3 id="pending-transactions-heading">Waiting to sync · {pending.length}</h3><div className="list">{pending.map((item) => <PendingExpenseRow key={item.clientOperationId} item={item} groupName={!groupId ? groupNames.get(item.groupId) : undefined} />)}</div></section> : null}{pageState && rows.length ? <ul className="list transaction-list">{rows.map((transaction) => <li key={transactionKey(transaction)}><TransactionRow key={transactionKey(transaction)} groupId={transaction.groupId} transaction={transaction} currentPersonId={me.data?.personId} showNotes /></li>)}</ul> : pageState && !pending.length ? <Empty>No transactions match the current filters.</Empty> : null}{pageState?.cursor ? <Button type="button" variant="secondary" disabled={loadingMore || offline} onClick={() => void loadMore()}>{loadingMore ? 'Loading…' : 'Load more transactions'}</Button> : null}{pageState?.pageError ? <ErrorBox error={pageState.pageError} id="history-transactions-page-error" /> : null}
...
function HistoryChanges({ groupId }: { groupId?: string }) {
  ...
  return <section className="history-panel" aria-labelledby="history-changes-heading"><h2 id="history-changes-heading" className="sr-only">Changes</h2>{!online || activity.offline ? <p className="offline-banner" role="status">Offline · showing cached history; this is only the first cached page.</p> : null}{me.error ? <CachedIdentityNotice resource={me} id="history-identity-error" /> : null}<ResourceNotice resource={activity} label="history" retry={retryFor(resourceKeys.activity(me.data?.id || 'pending', groupId || 'all'), me.data?.id)} />{activity.data !== undefined && items.length ? <ul className="list reading-width activity-list">{items.map((item) => <li key={`${item.type}-${item.id}`}>{itemRow(item)}</li>)}</ul> : activity.data !== undefined ? <Empty>No history yet.</Empty> : null}{cursor ? <Button type="button" variant="secondary" disabled={loadingMore || !online} onClick={() => void loadMore()}>{loadingMore ? 'Loading…' : 'Load more history'}</Button> : null}{pageError ? <ErrorBox error={pageError} id="history-page-error" /> : null}</section>;
}
