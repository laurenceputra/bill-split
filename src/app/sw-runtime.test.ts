...
function workerHarness(cacheMatch: (request: Request | string) => Promise<Response | undefined>, networkFetch: (request?: Request) => Promise<Response> = async () => { throw new Error('unexpected network request'); }, cachePut = () => new Promise<void>(() => undefined), workerIndexedDb = indexedDB, badgeNavigator?: { setAppBadge?: (count: number) => Promise<void>; clearAppBadge?: () => Promise<void> }) {
  ...
  return { fetchHandler: handlers.get('fetch')!, installHandler: handlers.get('install')!, pushHandler: handlers.get('push')!, clickHandler: handlers.get('notificationclick')!, syncHandler: handlers.get('sync')!, messageHandler: handlers.get('message')!, waitUntilPromises, shown, badgeCounts, badgeClears, windows };
}
...
describe('service-worker runtime policy', () => {
  ...
  it('rejects malformed push payloads and sanitizes a notification click route', async () => {
    ...
    harness.messageHandler({ data: { type: 'CLEAR_NOTIFICATION_BADGE' }, waitUntil: (promise: Promise<unknown>) => { clearPromise = promise; } });
    await clearPromise;
    expect(harness.badgeClears).toHaveLength(1);
  ...
  it('keeps valid pushes non-fatal when the badging API is unavailable', async () => {
    await new Promise<void>((resolve) => { const request = indexedDB.deleteDatabase('bill-split-notification-state'); request.onsuccess = request.onerror = () => resolve(); request.onblocked = () => undefined; });
    await new Promise<void>((resolve, reject) => {
      const request = indexedDB.open('bill-split-notification-state', 1);
      request.onupgradeneeded = () => { if (!request.result.objectStoreNames.contains('identity')) request.result.createObjectStore('identity', { keyPath: 'key' }); if (!request.result.objectStoreNames.contains('badge')) request.result.createObjectStore('badge', { keyPath: 'key' }); };
    ...
    const harness = workerHarness(async () => undefined, undefined, undefined, indexedDB, {});
    ...
    await pushPromise;
    expect(harness.shown).toHaveLength(1);
    expect(harness.badgeCounts).toHaveLength(0);
    expect(harness.badgeClears).toHaveLength(0);
  });
  ...
  it('closes a notification database that opens after the fail-closed timeout', async () => {
    vi.useFakeTimers();
    try {
      const close = vi.fn();
      const request: { result: { close: () => void }; onsuccess?: () => void } = { result: { close } };
      const delayedIndexedDb = { open: vi.fn(() => { setTimeout(() => request.onsuccess?.(), 1_000); return request; }) } as unknown as IDBFactory;
      const harness = workerHarness(async () => undefined, undefined, undefined, delayedIndexedDb);
      let pushPromise!: Promise<unknown>;
      harness.pushHandler({ data: { json: () => ({ title: 'x', body: 'x', data: { eventId: 'event-1', recipientUserId: 'user-1', route: '/activity' } }) }, waitUntil: (promise: Promise<unknown>) => { pushPromise = promise; } });
