...
function coordinateAuthBootstrapImpl(evidence: ClerkAuthEvidence, options: { networkOnly?: boolean; startupFallbackMs?: number; force?: boolean; route?: AuthBootstrapRoute } = {}) {
  ...
  if (evidenceChanged) {
    ...
    // account still advances the API epoch to fence in-flight requests.
    advanceAuthEpoch();
    cancelClerkRestorationDeadline();
...
/** Insight aggregates are intentionally not served from the bounded transaction cache. */
export async function getSpendingInsightSummary(groupId?: string, options: SpendingInsightSummaryOptions = {}, signal?: AbortSignal): Promise<CachedResult<SpendingInsightSummaryResponse>> {
  ...
  if (options.comparisonFrom) query.set('comparisonFrom', options.comparisonFrom);
  if (options.comparisonTo) query.set('comparisonTo', options.comparisonTo);
  try {
    return (await apiWithMeta<SpendingInsightSummaryResponse>(`/spending-insights?${query}`, { signal })).data;
  } catch (error) {
...
export async function getSpendingInsightTrends(groupId: string | undefined, options: SpendingInsightTrendOptions, signal?: AbortSignal): Promise<CachedResult<SpendingInsightTrends>> {
  const query = new URLSearchParams();
  if (groupId) query.set('group', groupId);
  query.set('view', 'trends');
  query.set('trendFrom', options.trendFrom);
  query.set('trendTo', options.trendTo);
  if (options.currency) query.set('currency', options.currency);
  try {
    return (await apiWithMeta<SpendingInsightTrends>(`/spending-insights?${query}`, { signal })).data;
  } catch (error) {
    if (groupId && isGroupAuthorizationLoss(error)) await evictRevokedGroupForCurrentUser(groupId);
    throw error;
  }
...
if (typeof window !== 'undefined') window.addEventListener('billsplit-cache-cleared', (event) => {
  ...
  // dispatching this UI/cache event. Only the manual local Settings
