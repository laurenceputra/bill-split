...
import { accountDeletionInput, categorySuggestionInput, currency, date, friendInput, groupCreationInput, groupConversionInput, groupInput, groupSplitDefaultInput, invitationInput, ownershipTransferInput, personInput, profileNameInput } from '../shared/schemas';
...
const repositoryError = (c: any, error: unknown) => {
    if (error instanceof RepositoryError) return jsonError(c, error.code === 'BALANCE_OVERFLOW' ? 422 : error.code === 'OWNER_REQUIRED' ? 403 : error.code === 'CONFLICT' || error.code === 'IDEMPOTENCY_CONFLICT' || error.code === 'AUTH_IDENTITY_CONFLICT' || error.code === 'FINAL_OWNER' || error.code === 'INVITATION_EXPIRED' || error.code === 'INVITATION_REVOKED' || error.code === 'PEER_LIMIT' || error.code === 'ACCOUNT_DELETION_BLOCKED' || error.code === 'CREDIT_EXPENSE_LINKED' ? 409 : error.code === 'SELF_FRIEND' || error.code === 'INVITATION_INVALID' || error.code === 'MEMBER_REQUIRED' || error.code === 'INVALID_SEARCH' || error.code === 'INVALID_CURSOR' || error.code === 'INVALID_PAGINATION' || error.code === 'INVALID_DATE' || error.code === 'INVALID_FILTER' || error.code === 'INVALID_SPLIT_DEFAULT' || error.code === 'INVALID_CREDIT' ? 400 : 500, error.code, error.message, error.details);
  throw error;
};
...
api.post('/api/groups', zValidator('json', groupCreationInput), async (c) => { try { return c.json({ group: await getRepo(c).createGroup(c.get('auth').id, c.get('auth').personId, c.req.valid('json')) }, 201); } catch (error) { return repositoryError(c, error); } });
...
api.put('/api/groups/:groupId', zValidator('json', groupInput), async (c) => { const x = await authorizedGroup(c, c.req.param('groupId')); if (x instanceof Response) return x; const denied = ownerOnly(c, x); if (denied) return denied; try { return c.json({ group: await x.repo.updateGroup(c.req.param('groupId'), x.auth.id, c.req.valid('json')) }); } catch (error) { return repositoryError(c, error); } });
api.post('/api/groups/:groupId/convert-to-named', zValidator('json', groupConversionInput), async (c) => { const x = await authorizedGroup(c, c.req.param('groupId')); if (x instanceof Response) return x; const denied = ownerOnly(c, x); if (denied) return denied; try { return c.json({ group: await x.repo.convertPeerToNamed(c.req.param('groupId'), x.auth.id, c.req.valid('json').name) }); } catch (error) { return repositoryError(c, error); } });
api.post('/api/groups/:groupId/convert-to-peer', async (c) => { const x = await authorizedGroup(c, c.req.param('groupId')); if (x instanceof Response) return x; const denied = ownerOnly(c, x); if (denied) return denied; try { return c.json({ group: await x.repo.convertNamedToPeer(c.req.param('groupId'), x.auth.id) }); } catch (error) { return repositoryError(c, error); } });
...
api.get('/api/transactions', async (c) => {
  ...
  if (q.kind !== undefined && q.kind !== 'expense' && q.kind !== 'settlement' && q.kind !== 'credit') return jsonError(c, 400, 'INVALID_FILTER', 'Transaction kind must be expense, settlement, or credit');
...
api.get('/api/groups/:groupId/audit/:entityType/:entityId', async (c) => {
  ...
  const entityType = c.req.param('entityType');
   if (entityType !== 'expense' && entityType !== 'settlement' && entityType !== 'credit') return jsonError(c, 400, 'INVALID_AUDIT_ENTITY', 'Audit entity type must be expense, settlement, or credit');
  const offsetError = rejectOffset(c); if (offsetError) return offsetError;
...
api.get('/api/categories', async (c) => c.json({ categories: await getRepo(c).categories(c.get('auth').id) }));
  api.get('/api/groups/:groupId/export.json', async (c) => { const x = await authorizedGroup(c, c.req.param('groupId')); if (x instanceof Response) return x; const q = c.req.query(), limit = page(q.limit, 50, 100); if (limit < 1) return jsonError(c, 400, 'INVALID_PAGINATION', 'Pagination values must be finite non-negative integers'); try { return c.json(await x.repo.groupExportPage(c.req.param('groupId'), { limit, expenseCursor: q.expenseDone === '1' ? null : q.expenseCursor, settlementCursor: q.settlementDone === '1' ? null : q.settlementCursor, creditCursor: q.creditDone === '1' ? null : q.creditCursor })); } catch (error) { return repositoryError(c, error); } });
   api.get('/api/groups/:groupId/export.csv', async (c) => { const x = await authorizedGroup(c, c.req.param('groupId')); if (x instanceof Response) return x; const offsetError = rejectOffset(c); if (offsetError) return offsetError; const q = c.req.query(), limit = page(q.limit, 100, 100); if (limit < 1) return jsonError(c, 400, 'INVALID_PAGINATION', 'Pagination values must be finite non-negative integers'); try { const result = await x.repo.expensePage(c.req.param('groupId'), { limit, cursor: q.cursor }); const csv = ['date,description,amount_minor,currency,payers,splits', ...result.items.map((expense) => [expense.date, expense.description, expense.amountMinor, expense.currency, expense.payers.map((p) => `${p.personId}:${p.amountMinor}`).join(';'), expense.splits.map((s) => `${s.personId}:${s.amountMinor}`).join(';')].map(escapeCsvCell).join(','))].join('\n'); const headers: Record<string, string> = { 'Content-Type': 'text/csv; charset=utf-8', 'Content-Disposition': 'attachment; filename="group-expenses.csv"' }; if (result.nextCursor) headers['X-Next-Cursor'] = result.nextCursor; return new Response(csv, { headers }); } catch (error) { return repositoryError(c, error); } });
    ...
    api.get('/api/groups/:groupId/credits.csv', async (c) => { const x = await authorizedGroup(c, c.req.param('groupId')); if (x instanceof Response) return x; const offsetError = rejectOffset(c); if (offsetError) return offsetError; const q = c.req.query(), limit = page(q.limit, 100, 100); if (limit < 1) return jsonError(c, 400, 'INVALID_PAGINATION', 'Pagination values must be finite non-negative integers'); try { const result = await x.repo.creditPage(c.req.param('groupId'), { limit, cursor: q.cursor }); const csv = ['date,subtype,delivery_mode,amount_minor,currency,applications,allocations,note', ...result.items.map((credit) => [credit.date, credit.subtype, credit.deliveryMode, credit.amountMinor, credit.currency, credit.applications.map((item) => `${item.expenseId}:${item.amountMinor}`).join(';'), credit.allocations.map((item) => `${item.allocationType}:${item.personId}:${item.amountMinor}`).join(';'), credit.note].map(escapeCsvCell).join(','))].join('\n'); const headers: Record<string, string> = { 'Content-Type': 'text/csv; charset=utf-8', 'Content-Disposition': 'attachment; filename="group-credits.csv"' }; if (result.nextCursor) headers['X-Next-Cursor'] = result.nextCursor; return new Response(csv, { headers }); } catch (error) { return repositoryError(c, error); } });
