...
function mapGroup(row: Row | null): Group | null {
  if (!row) return null;
  return { id: text(row.id), name: text(row.name), currency: currency(row.currency), createdAt: text(row.created_at), updatedAt: text(row.updated_at), ...(row.role ? { role: text(row.role) as Group['role'] } : {}) };
}
...
export class Repository {
  ...
  async groups(userId: string): Promise<Group[]> {
    const rows = (await this.db.prepare('SELECT g.*, gm.role FROM groups g JOIN group_members gm ON gm.group_id=g.id WHERE gm.user_id=? AND gm.deleted_at IS NULL AND g.deleted_at IS NULL ORDER BY g.created_at DESC').bind(userId).all<Row>()).results;
    return rows.map((row) => mapGroup(row)!).filter(Boolean);
  }
  async group(groupId: string, userId: string): Promise<Group | null> {
    return mapGroup(await this.db.prepare('SELECT g.*, gm.role FROM groups g JOIN group_members gm ON gm.group_id=g.id WHERE g.id=? AND gm.user_id=? AND gm.deleted_at IS NULL AND g.deleted_at IS NULL').bind(groupId, userId).first<Row>());
  }
  ...
  async addPerson(groupId: string, person: { name: string; email?: string | null }) {
  ...
  async activity(groupId: string) {
    const rows = (await this.db.prepare(`SELECT 'expense' AS type,id,description AS label,created_at FROM expenses WHERE group_id=? AND deleted_at IS NULL UNION ALL SELECT 'settlement' AS type,id,note AS label,created_at FROM settlements WHERE group_id=? AND deleted_at IS NULL UNION ALL SELECT CASE WHEN (r.entity_type='expense' AND e.deleted_at IS NULL) OR (r.entity_type='settlement' AND s.deleted_at IS NULL) THEN r.entity_type || '_revision' ELSE r.entity_type || '_deleted' END AS type,r.id,CASE WHEN (r.entity_type='expense' AND e.deleted_at IS NULL) OR (r.entity_type='settlement' AND s.deleted_at IS NULL) THEN 'Record edited' ELSE 'Record deleted' END AS label,r.created_at FROM revisions r LEFT JOIN expenses e ON r.entity_type='expense' AND e.id=r.entity_id LEFT JOIN settlements s ON r.entity_type='settlement' AND s.id=r.entity_id WHERE (e.group_id=? OR s.group_id=?) ORDER BY created_at DESC LIMIT 100`).bind(groupId, groupId, groupId, groupId).all<Row>()).results;
    return rows.map((row) => ({ type: text(row.type), id: text(row.id), label: row.label == null ? null : text(row.label), createdAt: text(row.created_at) }));
  }
